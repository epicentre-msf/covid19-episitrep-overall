...
group_by(epi_week_consultation) %>% 
  summarise(n_patients_PCR = sum(merge_test_PCR == "Yes", na.rm = TRUE),
            ...
            perct_patient_pos = ifelse(n_patients_PCR == 0,
                               NA_integer_,
                               100 * n_patients_PCR_positive / n_patients_PCR),
            ...
            perct_test_pos = ifelse(nb_test_PCR == 0,
                                    NA_integer_,
                                    100 * nb_tests_PCR_positive / nb_test_PCR)) %>% 
  
  ggplot(aes(x = epi_week_consultation)) +
  geom_col(aes(y = n_patients_PCR),
           fill = "#A0CBE8") +
  geom_line(aes(y = perct_patient_pos),
            colour = "red") +
...
dta_linelist %>%
  ...
  mutate(severity = ifelse(is.na(severity), "Non available", severity),
         severity = factor(severity, 
                           levels = c("Mild", "Moderate", "Severe", "Critical", "Non available"))) %>% 
  filter(outcome_status == "Died") %>%
...
plot_mortality_project <- function(data, 
  ...
  ggsave(filename = paste0(country, '_', 'mortality_severity', '_', week_report, '.png'),
         path = path_save,
         plot = fig_data, 
         width = 6,
         height = 4,
...
tbl_mortality_severity_p <- dta_linelist %>%
...
plot_mortality_admission_project <- function(data, 
  ...
  fig_data <- data %>% 
    ...
    labs(x = "Week of admission",
         y = "Nb of death",
         title = glue::glue("Mortality for hospitalized patients"))
    # ggthemes::scale_colour_tableau(name = "Severity", 
    #                                palette = "Tableau 20")
  
  ggsave(filename = paste0(country, '_', 'mortality_admission', '_', week_report, '.png'),
         path = path_save,
         plot = fig_data, 
         width = 6,
         height = 4,
...
tbl_mortality_admission_p <- dta_linelist %>%
  filter(country %in% c("Afghanistan", "Bangladesh", "Haiti", "Iraq", "Yemen"),
         covid_status %in% c('Confirmed', 'Probable', 'Suspected')) %>% 
  filter(outcome_status == "Died") %>%
  filter(merge_admit == "Yes") %>% 
