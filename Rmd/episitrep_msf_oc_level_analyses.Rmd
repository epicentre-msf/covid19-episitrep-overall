...
path.local.week.oc   <- file.path(path.local.oc, week_report)
...
# Additional analyses asked by OCP
...
tbl_diff_hospi <- dta_linelist %>%
  ...
  summarise(n_nonadmited = sum(merge_admit == "No"),
            m_nonadmited = mean(merge_admit == "No"),
            n_admited    = sum(merge_admit == "Yes"),
            m_admited    = mean(merge_admit == "Yes")) %>% 
...
gtbl_diff_hospi <- tbl_diff_hospi %>% 
  ...
  tab_spanner(
    label = 'Hospitalized',
    columns = ends_with('_Hospitalized')) %>%
...
gtbl_diff_hospi %>% 
  tab_header(
    title = 'Mean age x gender x hospitalization (for confirmed, probable and suspect)')
