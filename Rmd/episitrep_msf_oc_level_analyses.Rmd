...
dta_linelist <- covidutils::get_msf_linelist(force = TRUE) %>%
  select(-Comcond_01, -Comcond_count) %>% 
  covidutils::prepare_msf_dta(shorten_var_names = FALSE) %>%
  rename(iso_a3        = country,
         Comcond_01    = ind_Comcond_01,
         Comcond_count = ind_Comcond_count) %>%
...
dta_linelist_project <- dta_linelist %>%
  ...
  mutate(severity = ifelse(is.na(severity), "Non available", severity),
...
dta_linelist_with_aggregated <- dta_linelist %>%
  select(continent, region, country, site_name, iso_a3, OC,
         epi_week_consultation, covid_status, project) %>%
...
nb_msf_confirmed_with_comorbidity <- with(dta_linelist,
                                          sum(covid_status == 'Confirmed' & ind_Comcond_count > 0,
                                              na.rm = TRUE))

nb_msf_conf_prob_susp <- with(dta_linelist,
                              sum(covid_status %in% c('Confirmed', 'Probable', 'Suspected') & outcome_status %in% c('Cured', 'Died'), na.rm = TRUE))

nb_msf_conf_prob_susp_who_died <- with(dta_linelist,
                                       sum(covid_status %in% c('Confirmed', 'Probable', 'Suspected') & outcome_status == 'Died', na.rm = TRUE))

nb_msf_conf_above65 <- with(dta_linelist,
                            sum(covid_status == 'Confirmed' & age_in_years >= 65 & outcome_status %in% c('Cured', 'Died'), na.rm = TRUE))

nb_msf_conf_above65_who_died <- with(dta_linelist,
                                     sum(covid_status == 'Confirmed' & age_in_years >= 65 & outcome_status == 'Died', na.rm = TRUE))
...
dta_testing <- dta_linelist %>%
  select(country, project, epi_week_consultation, merge_admit, , covid_status,
         outcome_status, matches("test|lab|sample")) %>%
  rowwise %>%
  mutate(
...
tbl_general_by_status <- dta_linelist %>%
  select(covid_status, age_in_years, age_5gp, sex, ind_Comcond_01) %>%
...
dta_cfr_comcond <- prepare_msf_dta_comcond(dta_linelist) %>%
  ...
  pivot_longer(-c(covid_status, outcome_status, continent, ind_Comcond_count, ind_Comcond_01), names_to = "type_comorbidity") %>%
  mutate(
    type_comorbidity = factor(type_comorbidity, levels = df_labels_comcond$levels, labels = df_labels_comcond$labels)) %>%
...
```{r fig-severity-mortality}
dta_linelist_project %>%
  filter(outcome_status == "Died") %>%
  filter(merge_admit == "Yes") %>%
  
  group_by(project, epi_week_admission, severity) %>%
  summarise(n = n()) %>%
  ...
  labs(x = "Week of admission",
       y = "Nb of death",
       title = glue::glue("Mortality by severity for hospitalized patients")) +
  ggthemes::scale_colour_tableau(name = "Severity",
                                 palette = "Tableau 20")

ggsave(filename = paste0('mortality_severity', '_', week_report, '.png'),
       path = path.local.msf.graphs.oc,
       width = 8,
       height = 4,
       scale = 1.1,
...
tble16 <- dta_linelist_project %>%
  ...
  summarise(
    ...
    n_comorb_death = sum(ind_Comcond_01 == 1 & outcome_status == "Died", na.rm = TRUE),
...
fct_plot_2 <- function(data, 
  ...
  data <- data %>% 
    filter(outcome_status == "Died") %>%
    group_by(epi_week_admission, severity) %>%
    summarise(n = n()) %>%
    drop_na(severity)
  ...
  if(nrow(data) > 0) {
    ...
    ggsave(filename = paste0(country_project, '_', 'mortality_severity', '_', week_report, '.png'),
           path = path_save,
           plot = plot_1,
           width = 8,
           height = 5,
...
tbl_plot1 <- dta_linelist_with_aggregated %>%
  ...
  count(country_project, covid_status, epi_week_consultation) %>%
  drop_na(epi_week_consultation) %>% 
