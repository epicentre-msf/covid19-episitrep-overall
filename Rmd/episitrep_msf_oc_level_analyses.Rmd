...
dta_linelist_with_aggregated <- dta_linelist %>% 
  select(continent, region, country, site_name, iso_a3, OC, epi_week_consultation, covid_status, project) %>% 
...
## Figure 1 – Weekly positivity rates by project

```{r Fig1, fig.cap = 'Weekly number of consultations (including admission) by Covid status'}
...
plot_epicurve_project <- function(data, 
  ...
  ggsave(filename = paste0('hist_epicurve_status', '_', week_report, '_', continent, '.png'),
         path = path.local.msf.graphs.oc,
         plot = fig_data, 
         # width = 6, 
         # height = 4, 
...
tbl_epicurve_status_p <- dta_linelist_with_aggregated %>%
  count(continent, country, site_name, covid_status, epi_week_consultation) %>% 
  group_by(continent) %>% 
  nest() %>% 
  mutate(plot = map2(.x = data, 
                     ...
                     ~plot_epicurve_project(data = .x,
                                            ncols_facet = 3,
                                            continent = .y,
                                            path_save = path.local.msf.graphs.oc))) 
...
gtble1 %>% 
  tab_header(
    title = 'Number of patients hospitalised at admission or over the course of the disease')
```
...
gtble3 %>%
  tab_header(
    title = 'Length of stay according to severity')
```
...
```{r fig4, fig.cap="Median [IQR] time between onset of symptoms and presentations"}
fig4 <- dta_linelist %>%
  filter(covid_status %in% c('Confirmed', 'Probable', 'Suspected')) %>%
  ...
  # select(country, project, merge_admit, severity, length_stay) %>%
  group_by(continent, country, site_name) %>%
  ...
  pivot_wider(names_from = q, 
              values_from =x,
              names_prefix = "q_") %>% 
  ggplot(aes(x = fct_reorder(site_name, desc(q_0.5)),
             y = q_0.5,
             colour = continent)) +
  ...
  scale_size_continuous(name = "Number of presentations") +
  theme_light() + 
...
gtsave(gtble4, 
       file.path(path.local.msf.tables.oc, paste0('gtble4', '_', week_report, '.html')), 
       inline_css = TRUE) %>% 
  invisible()
...
gtsave(gtble5, 
       file.path(path.local.msf.tables.oc, paste0('gtble5', '_', week_report, '.html')), 
       inline_css = TRUE) %>% 
  invisible()
...
gtble12 %>% 
  tab_header(
    title = 'Total number of patient visits by case status and vent admission status')
```
...
```{r fig5, fig.cap = 'Median and interquartile range of patient age by sex and project'}
fig5 <- dta_linelist %>%
  filter(covid_status %in% c('Confirmed', 'Probable', 'Suspected')) %>%
  drop_na(sex) %>% 
  ...
  group_by(continent, country, site_name, Sex) %>%
...
```{r tble15}
list_comorb <- c("Comcond_diabetes", "hypertension", "Comcond_renal")


tble15 <- dta_linelist %>%
  filter(covid_status %in% c('Confirmed', 'Probable', 'Suspected')) %>%
  select(country, project, merge_admit, Comcond_01, Comcond_diabetes, hypertension,
         Comcond_renal, outcome_status) %>%
  group_by(country, project) %>% 
  summarise(n_comorb = sum(Comcond_01 == 1, na.rm = TRUE),
            p_comorb = n_comorb / n(),
            a_comorb = format_nbp(n_comorb, p_comorb),
            
            n_comorb_hospi = sum(Comcond_01 == 1 & merge_admit == "Yes", na.rm = TRUE), 
            ...
            p_comorb_death = n_comorb_death / n_comorb,
            ...
            a_diabetes_hospi = format_nbp(n_diabetes_hospi, p_diabetes),
            
            n_diabetes_death = sum(Comcond_diabetes == "Yes" & outcome_status == "Died", na.rm = TRUE),
            p_diabetes_death = n_diabetes_death / n_diabetes,
            a_diabetes_death = format_nbp(n_diabetes_death, p_diabetes),
            ...
            a_hypertension_hospi  = format_nbp(n_hypertension_hospi, p_hypertension),
            
            n_hypertension_death  = sum(hypertension == "Yes" & outcome_status == "Died", na.rm = TRUE),
            p_hypertension_death  = n_hypertension_death / n_hypertension,
            a_hypertension_death  = format_nbp(n_hypertension_death, p_hypertension),
            ...
            a_chronic_renal_hospi = format_nbp(n_chronic_renal_hospi, p_chronic_renal),
            
            n_chronic_renal_death = sum(Comcond_renal == "Yes" & outcome_status == "Died", na.rm = TRUE),
            p_chronic_renal_death = n_chronic_renal_death / n_chronic_renal,
            a_chronic_renal_death = format_nbp(n_chronic_renal_death, p_chronic_renal)
  ) %>% 
...
list_coinf <- c("malaria", "hiv_status", "tb_active", "tb_treatment_past")


tble16 <- dta_linelist %>%
  filter(covid_status %in% c('Confirmed', 'Probable', 'Suspected')) %>%
  select(country, project, merge_admit, outcome_status, malaria, hiv_status, tb_active, tb_treatment_past) %>%
  group_by(country, project) %>% 
  mutate(coinf = ifelse(malaria == "Yes" | hiv_status == "Yes" | tb_active == "Yes" | tb_treatment_past == "Yes",
                        1, 0)) %>% 
...
```{r fig6, fig.cap = 'Mortality by therapy received, by project'}
fig6
dta_fig6 <- dta_linelist %>%
  filter(covid_status %in% c('Confirmed', 'Probable', 'Suspected'),
     merge_admit == "Yes") %>% 
  mutate(therapy = case_when(
    merge_oxygen == "Yes" & merge_icu != "Yes" & merge_vent != "Yes" ~ "Oxygen",
    merge_oxygen != "Yes" & merge_icu == "Yes" & merge_vent != "Yes" ~ "ICU",
    merge_oxygen != "Yes" & merge_icu != "Yes" & merge_vent == "Yes" ~ "Ventilation",
    merge_oxygen == "Yes" & merge_icu == "Yes" & merge_vent != "Yes" ~ "Oxygen-ICU",
    merge_oxygen == "Yes" & merge_icu != "Yes" & merge_vent == "Yes" ~ "Oxygen-Ventilation",
    merge_oxygen != "Yes" & merge_icu == "Yes" & merge_vent == "Yes" ~ "ICU-Ventilation",
    merge_oxygen == "Yes" & merge_icu == "Yes" & merge_vent == "Yes" ~ "Oxygen-ICU-Ventilation",
    merge_oxygen == "No at any time" & merge_icu == "No at any time" & merge_vent == "No at any time" ~ "Not at any time",
    TRUE ~ "Unknown"),
    
    therapy = factor(x = therapy,
                        level = c("Oxygen", "ICU", "Ventilation", "Oxygen-ICU", 
                                   "Oxygen-Ventilation", "ICU-Ventilation", 
                                   "Oxygen-ICU-Ventilation","Not at any time", 
                                  "Unknown"))
    ) %>% 
  group_by(continent, country, project, therapy) %>%
  summarise(
    n_death = sum(outcome_status == "Died", na.rm = TRUE),
    n_admit = n(),
    p = round(100 * n_death/n_admit, 1)
  ) 
...
```{r gtbl-diff-hospi}
tbl_diff_hospi <- dta_linelist %>%
  filter(covid_status %in% c('Confirmed', 'Probable', 'Suspected')) %>%
  select(country, project, sex, age_in_years, merge_admit) %>%
  drop_na(merge_admit) %>% 
  group_by(country, project, sex) %>% 
  summarise(n_nonadmited = sum(merge_admit == "No"),
            m_nonadmited = mean(age_in_years [merge_admit == "No"], 
                                na.rm = TRUE),
            n_admited    = sum(merge_admit == "Yes"),
            m_admited    = mean(age_in_years [merge_admit == "Yes"], 
                                na.rm = TRUE)) %>% 
  mutate(sex = recode(sex, "M" = "Male", "F" = "Female")) %>% 
  group_by(country)


gtbl_diff_hospi <- tbl_diff_hospi %>% 
  gt() %>%
  ...
  cols_label(
    ...
    sex          = 'Sex',
    n_nonadmited = 'N', 
    m_nonadmited = 'Mean age (yrs)', 
    n_admited    = 'N', 
    m_admited    = 'Mean age (yrs)') %>%
  
  tab_spanner(
    label = 'Non hospitalized',
    columns = ends_with('_nonadmited')) %>%
  tab_spanner(
    label = 'Hospitalized',
    columns = ends_with('_admited')) %>%
...
gtsave(gtbl_diff_hospi, 
       file.path(path.local.msf.tables.oc, paste0('gtbl_diff_hospi', '_', week_report, '.html')), 
       inline_css = TRUE) %>% 
  invisible()
...
gtbl_diff_hospi %>% 
  tab_header(
    title = 'Gender and age of hospitalized and non-hospitalized patients')
```



```{r tbl-deaths-hospi-icu}
tbl_deaths_hospi_icu <- dta_linelist %>%
  filter(covid_status %in% c('Confirmed', 'Probable', 'Suspected')) %>%
  select(country, project, merge_admit, outcome_status, merge_icu) %>%
  group_by(country, project) %>% 
  summarise(n_death = sum(outcome_status == "Died", na.rm = TRUE),
            n_death_hospi = sum(outcome_status == "Died" & merge_admit == "Yes", na.rm = TRUE),
            p_death_hospi = n_death_hospi / n_death,
            n_death_icu   = sum(outcome_status == "Died" & merge_icu == "Yes", na.rm = TRUE),
            p_death_icu   = n_death_icu / n_death)


gtbl_deaths_hospi_icu <- tbl_deaths_hospi_icu %>% 
  gt() %>%
  cols_label(
    country       = 'Country',
    project       = 'Project',
    n_death       = 'N deaths',
    n_death_hospi = 'N',
    p_death_hospi = '% of deaths', 
    n_death_icu   = 'N',
    p_death_icu   = '% of deaths') %>%
  
  tab_spanner(
    label = 'Hospitalized',
    columns = ends_with('_hospi')) %>%
  tab_spanner(
    label = 'ICU',
    columns = ends_with('_icu')) %>%
...
gtsave(gtbl_deaths_hospi_icu, 
       file.path(path.local.msf.tables.oc, paste0('gtbl_deaths_hospi_icu', '_',
                                                  week_report, '.html')), 
       inline_css = TRUE) %>% 
  invisible()


gtbl_deaths_hospi_icu %>% 
  tab_header(
    title = 'Number of deaths (total, in hospital and in ICU)')
```


```{r tbl-patients-o2-vent}
tbl_patients_o2_vent <- dta_linelist %>%
  filter(covid_status %in% c('Confirmed', 'Probable', 'Suspected')) %>%
  select(country, project, merge_admit, merge_vent, merge_oxygen) %>%
  group_by(country, project) %>% 
  # filter(merge_admit == "Yes") %>% 
  summarise(n_patients    = n(),
            n_patients_admis = sum(merge_admit == "Yes", na.rm = TRUE),
            n_oxygen         = sum(merge_oxygen == "Yes", na.rm = TRUE),
            p_oxygen         = n_oxygen / n_patients_admis,
            n_ventilation    = sum(merge_vent == "Yes", na.rm = TRUE),
            p_ventilation    = n_ventilation / n_patients_admis)


gtbl_patients_o2_vent <- tbl_patients_o2_vent %>% 
  gt() %>%
  cols_label(
    country          = 'Country',
    project          = 'Project',
    n_patients       = 'Total patients',
    n_patients_admis = 'N hospitalized',
    n_oxygen         = 'N under oxygen',
    p_oxygen         = '% O2',
    n_ventilation    = 'N ventilated',
    p_ventilation    = '% ventilated') %>%
...
gtsave(gtbl_patients_o2_vent, 
       file.path(path.local.msf.tables.oc, paste0('gtbl_patients_o2_vent', '_', week_report, '.html')), 
       inline_css = TRUE) %>% 
  invisible()


gtbl_patients_o2_vent %>% 
  tab_header(
    title = 'Number of confirmed, probable and suspected patients hospitalized, with type of care')
```
...
```{r save_results}
rm(list = lsf.str())
save.image(file.path(path.local.msf.data.oc, paste0('episitrep_msf_level_analyses', '_', week_report, '.RData')))
```
