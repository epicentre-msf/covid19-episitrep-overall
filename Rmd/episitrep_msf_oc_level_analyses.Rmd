...
list_coinf <- c("malaria", "hiv_status", "tb_active")


tble16 <- dta_linelist %>%
  filter(covid_status %in% c('Confirmed', 'Probable', 'Suspected')) %>%
  select(country, project, merge_admit, outcome_status, malaria, hiv_status, tb_active) %>%
  group_by(country, project) %>% 
  ...
  summarise(n_coinf = sum(coinf == 1, na.rm = TRUE),
            ...
            n_tb = sum(tb_active == "Yes", na.rm = TRUE),
            p_tb = n_tb / n_coinf,
            a_tb = format_nbp(n_tb, p_tb),
            
            n_tb_hospi = sum(tb_active == "Yes" & merge_admit == "Yes", na.rm = TRUE),
            p_tb_hospi = n_tb_hospi / n_tb,
            a_tb_hospi = format_nbp(n_tb_hospi, p_tb_hospi),
            
            n_tb_death = sum(tb_active == "Yes" & outcome_status == "Died", na.rm = TRUE),
            p_tb_death = n_tb_death / n_tb,
            a_tb_death = format_nbp(n_tb_death, p_tb_death)
