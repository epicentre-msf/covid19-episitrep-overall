...
dta_cfr_status_hospi_icu <- dta_linelist %>%
  ...
  filter(outcome_status %in% c('Cured', 'Died')) %>% 
  # filter(merge_admit != "(Unknown)") %>% 
  select(covid_status, merge_admit, outcome_status, merge_icu) %>%
  mutate(merge_admit = recode(merge_admit, "Yes" = "Admitted", "No" = "Not Admitted"))

  group_by(merge_admit, covid_status, merge_icu) %>% 
  summarise(
    total = n(),
    n_died = sum(outcome_status == 'Died'),
    n_other = sum(outcome_status != "Died")
  )
...
tbl_cfr_status_hospi_icu <- dta_cfr_status_hospi %>% 
...
gtbl_cfr_status_hospi_icu <- tbl_cfr_status_hospi_icu %>% 
  gt(rowname_col = "var", 
     groupname_col = "label_var") %>% 
  cols_hide(
...
gtsave(gtbl_cfr_status_hospi_icu, 
       file.path(path.local.msf.tables.oc, paste0('gtbl_cfr_status_hospi_icu', '_', week_report, '.html')), 
       inline_css = TRUE) %>% 
  invisible()


gtbl_cfr_status_hospi_icu %>% 
  tab_header(
