...
## Table 2 – Median length of stay according to severity
...
tble2 <- dta_linelist %>%
  ...
  summarise(n_patients = n(),
            ...
            p_mild     = n_mild / n_patients,
            m_mild     = median(length_stay[severity == "Mild"], na.rm = TRUE),
            
            n_moderate = sum(severity == "Moderate", na.rm = TRUE),
            p_moderate = n_moderate / n_patients,
            m_moderate = median(length_stay[severity == "Moderate"], na.rm = TRUE),
            
            n_severe   = sum(severity == "Severe", na.rm = TRUE),
            p_severe   = n_severe / n_patients,
            m_severe   = median(length_stay[severity == "Severe"], na.rm = TRUE),
            
            n_critical = sum(severity == "Critical", na.rm = TRUE),
            p_critical = n_critical / n_patients,
            m_critical = median(length_stay[severity == "Critical"], na.rm = TRUE),
...
gtble2 <- tble2 %>% 
  ...
  cols_label(
    ...
    n_moderate = 'N',
    p_moderate = "%",
    m_moderate = 'Median (days)',
    
    n_severe   = 'N',
    p_severe   = 'Median (days)',
    m_severe   = "%",
    
    n_critical = 'N',
    p_critical = "%",
    m_critical = 'Median (days)',
    
    n_na       = 'Severity non-available',
    p_na       = "%") %>% 
  
  tab_spanner(
    label = 'Mild',
    columns = ends_with('_mild')) %>%
  tab_spanner(
    label = 'Moderate',
    columns = ends_with('_moderate')) %>%
  tab_spanner(
    label = 'Severe',
    columns = ends_with('_severe')) %>%
  tab_spanner(
    label = 'Critical',
    columns = ends_with('_critical')) %>%
  tab_spanner(
    label = 'Missing data',
  ...
  summary_rows(
    columns = vars(n_patients, n_mild, n_moderate, n_critical, n_na),
    fns = list(Total = "sum"),
    decimals = 0)
...
## Table 3 – Median age stratified by gender, project and hospitalisation status


## Figure 2 – Median [IQR] of age per project, stratified by gender


## Table 4 – Total number of patient visits by ICU admission status, by project (includes confirmed, suspect, probable cases only)
...
tble4 <- dta_linelist %>%
  ...
  group_by(country, project) %>% 
  summarise(n_patients           = n(),
            n_admited      = sum(merge_admit == "Yes", na.rm = TRUE),
            p_admited      = n_admited / n_patients,
            
            n_icu_admission = sum(icu == "Yes", na.rm = TRUE),
            p_icu_admission = n_icu_admission / n_admited,
            
            n_icu_total = sum(merge_icu == "Yes", na.rm = TRUE),
            p_icu_total = n_icu_total / n_admited)
...
gtble4 <- 
  ...
  tab_spanner(
    label = 'Hospitalised',
    columns = ends_with('admited')) %>%
  
  tab_spanner(
    label = 'ICU at admission',
    columns = ends_with('_icu_admission')) %>%
  
  tab_spanner(
    label = 'ICU at any time',
    columns = ends_with('_icu_total')) %>%
  ...
  summary_rows(
    columns = vars(n_patients, n_admited, n_icu_admission, n_icu_total),
    fns = list(Total = "sum"),
    decimals = 0)
...
gtble4 %>% 
  tab_header(
    title = 'Total number of patient visits by ICU admission status, by project (includes confirmed, suspect, probable cases only)')
...
## Table 6 – Total number of patient visits patients admitted and patients given oxygen on admission and over course of disease (confirmed, suspect, probable cases only)

## Table 7 – Total number of patient visits patients admitted and patients given oxygen on admission and over course of disease, by case status 

## Table 8 – Total number of patient visits by Oxygen requirement, when applicable

## Table 9 – Total number of patient visits, patients admitted and patients mechanically ventilated at admission and over course of disease (confirmed, suspect, probable cases only)

## Table 10 – Case severity per project

## Table 11 – Mortality

## Table 12 – Number of patients with comorbidities and proportion of hospitalizations
