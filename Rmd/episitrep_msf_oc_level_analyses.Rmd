...
dta_aggregated_expanded <- dta_weekly_aggregated %>%
  ...
  pivot_longer(cols = c('confirmed', 'probable', 'suspected', 'non_cases', 'unknown'),
               names_to = 'MSF_covid_status') %>%
  filter(!is.na(value)) %>%
  mutate(obs = purrr::map(value, ~rep_len(1, .x))) %>%
  unnest(cols = c(obs)) %>%
  select(-c(value, obs)) %>%
  mutate(
    MSF_covid_status = factor(MSF_covid_status,
...
dta_linelist_with_aggregated <- dta_linelist %>%
  select(continent, region, country, site_name, iso_a3, OC,
         epi_week_consultation, MSF_covid_status, project) %>%
  add_row(dta_aggregated_expanded %>%
            select(continent, region, country, site_name, iso_a3,
                   OC, epi_week_consultation, MSF_covid_status) %>%
            mutate(
  ...
  mutate(
    project = case_when(
      ...
      TRUE           ~ project),
    MSF_covid_status = recode(MSF_covid_status, "(Unknown)" = "Unknown")
  )
...
nb_msf_confirmed <- sum(dta_linelist_with_aggregated$MSF_covid_status == 'Confirmed', na.rm = TRUE)
nb_msf_probable  <- sum(dta_linelist_with_aggregated$MSF_covid_status == 'Probable' , na.rm = TRUE)
nb_msf_suspected <- sum(dta_linelist_with_aggregated$MSF_covid_status == 'Suspected', na.rm = TRUE)


nb_msf_confirmed_linelist <- sum(dta_linelist$MSF_covid_status == 'Confirmed', na.rm = TRUE)

nb_msf_confirmed_with_comorbidity <- with(dta_linelist,
                                          sum(MSF_covid_status == 'Confirmed' & ind_Comcond_count > 0,
                                              na.rm = TRUE))

nb_msf_conf_prob_susp <- with(dta_linelist,
                              sum(MSF_covid_status %in% c('Confirmed', 'Probable', 'Suspected') & outcome_patcourse_status %in% c('Cured', 'Died'), na.rm = TRUE))

nb_msf_conf_prob_susp_who_died <- with(dta_linelist,
                                       sum(MSF_covid_status %in% c('Confirmed', 'Probable', 'Suspected') & outcome_patcourse_status == 'Died', na.rm = TRUE))

nb_msf_conf_above65 <- with(dta_linelist,
                            sum(MSF_covid_status == 'Confirmed' & age_in_years >= 65 & outcome_patcourse_status %in% c('Cured', 'Died'), na.rm = TRUE))

nb_msf_conf_above65_who_died <- with(dta_linelist,
                                     sum(MSF_covid_status == 'Confirmed' & age_in_years >= 65 & outcome_patcourse_status == 'Died', na.rm = TRUE))
...
median_age_confirmed_died <- dta_linelist %>%
  filter(MSF_covid_status == 'Confirmed' & outcome_patcourse_status == 'Died') %>%
  pull(age_in_years) %>%
  median(na.rm = TRUE)
```
...
tbl_epicurve_status <- dta_linelist_with_aggregated %>%
  count(MSF_covid_status, epi_week_consultation)
...
tbl_epicurve_status_continent <- dta_linelist_with_aggregated %>%
  count(continent, MSF_covid_status, epi_week_consultation)
...
dta_testing <- dta_linelist %>%
  select(country, project, epi_week_consultation, merge_admit, , MSF_covid_status,
         outcome_patcourse_status, matches("test|lab|sample")) %>%
  rowwise %>%
  mutate(
    test_1_10 = ifelse(test_type   == "PCR", 1, 0),
    test_2_10 = ifelse(test_type_2 == "PCR", 1, 0),
    test_3_10 = ifelse(test_type_3 == "PCR", 1, 0),
    test_4_10 = ifelse(test_type_4 == "PCR", 1, 0),
    n_test_PCR = sum(test_1_10, test_2_10, test_3_10, test_4_10, na.rm = TRUE),
    merge_test_PCR = ifelse(n_test_PCR >= 1, "Yes", "No"),
    
    test_result_1 = ifelse(test_type   == "PCR" & test_results       == "Positive", 1, 0),
    test_result_2 = ifelse(test_type_2 == "PCR" & test_results_2     == "Positive", 1, 0),
    test_result_3 = ifelse(test_type_3 == "PCR" & outcome_lab_result == "Positive", 1, 0),
    test_result_4 = ifelse(test_type_4 == "PCR" & test_results_4     == "Positive", 1, 0),
...
# DONE by Francesco
tbl_countries_MSF_covid_status <- dta_linelist_with_aggregated %>%
  select(continent, country, site_name, MSF_covid_status) %>%
  group_by(continent, country) %>%
  summarise(
    total = n(),
    n_confirmed = sum(MSF_covid_status == 'Confirmed'),
    n_probable  = sum(MSF_covid_status == 'Probable'),
    n_suspected = sum(MSF_covid_status == 'Suspected'),
    n_not_case  = sum(MSF_covid_status == 'Not a case'),
    n_unknown   = sum(MSF_covid_status == '(Unknown)'),
...
gtbl_countries_MSF_covid_status <- tbl_countries_MSF_covid_status %>%
...
gtsave(gtbl_countries_MSF_covid_status,
       file.path(path.local.msf.tables.oc, paste0('gtbl_countries_MSF_covid_status', '_', week_report, '.png'))) %>%
  invisible()
...
gtbl_countries_MSF_covid_status %>%
  tab_header(
...
tbl_sites_MSF_covid_status <- dta_linelist_with_aggregated %>%
...
gtbl_sites_MSF_covid_status <- tbl_sites_MSF_covid_status %>%
...
gtsave(gtbl_sites_MSF_covid_status,
       file.path(path.local.msf.tables.oc, paste0('gtbl_sites_MSF_covid_status', '_', week_report, '.png'))) %>%
  invisible()
...
dta_age_sex <- dta_linelist %>%
  select(MSF_covid_status, age_in_years, age_5gp, sex) %>%
  drop_na() %>%
...
tbl_general_by_status <- dta_linelist %>%
  select(MSF_covid_status, age_in_years, age_5gp, sex, ind_Comcond_01) %>%
  group_by(MSF_covid_status) %>%
  summarise(
...
tbl_general_total <- dta_linelist %>%
  ...
  summarise(
    ...
    comorbidity_presence = freq_prct(ind_Comcond_01, 1)) %>%
  mutate(
    MSF_covid_status = 'Total')

tbl_general <- tbl_general_by_status %>%
  add_row(tbl_general_total) %>%
  mutate(
    MSF_covid_status = factor(MSF_covid_status, levels = c('Confirmed', 'Probable', 'Suspected', 'Not a case', '(Unknown)', 'Total')))
...
gtbl_general <- tbl_general %>%
  gather(characteristics, values, -MSF_covid_status, factor_key = TRUE) %>%
  spread(MSF_covid_status, values) %>%
...
tbl_comcond_status <- names(dta_comcond)[4:ncol(dta_comcond)] %>%
  map_df(~{
    dta_comcond %>%
      drop_na(any_of(.x)) %>%
      tbl_prop(.x, 'MSF_covid_status', drop_levels = TRUE) %>%
      rename('value'= .x) %>%
      mutate(
        type_comorbidity = .x)
  }) %>%
...
dta_sympt <- dta_linelist %>%
  select(MSF_covid_status, tidyselect::vars_select(names(dta_linelist), starts_with('symptom_') & !ends_with('_date_onset')))
...
tbl_sympt_status <- names(dta_sympt)[2:ncol(dta_sympt)] %>%
  map_df(~{
    dta_sympt %>%
      drop_na(any_of(.x)) %>%
      tbl_prop(.x, 'MSF_covid_status', drop_levels = FALSE) %>%
      rename('value'= .x) %>%
      mutate(
        type_symptom = .x)
  }) %>%
...
dta_MSF_severity <- dta_linelist %>%
  select(MSF_covid_status, MSF_severity) %>%
  mutate(
...
tbl_MSF_severity_status <- dta_MSF_severity %>%
  ...
  tbl_prop('MSF_severity', 'MSF_covid_status', drop_levels = FALSE)
...
dta_MSF_severity_confirmed_continent <- dta_linelist %>%
  select(continent, MSF_covid_status, MSF_severity) %>%
  filter(MSF_covid_status == 'Confirmed') %>%
  mutate(
    MSF_severity = factor(MSF_severity, levels = c('Mild', 'Moderate', 'Severe', 'Critical'))
  )
...
dta_cfr_status <- dta_linelist %>%
  select(continent, MSF_covid_status, outcome_patcourse_status) %>%
  filter(outcome_patcourse_status %in% c('Cured', 'Died'))

tbl_cfr_status_continent <- levels(dta_cfr_status$MSF_covid_status) %>%
  map_df(~{
    dta_cfr_status %>%
      filter(MSF_covid_status == .x) %>%
      tbl_prop('outcome_patcourse_status', 'continent', drop_levels = FALSE) %>%
      mutate(
        MSF_covid_status = .x)
  }) %>%
...
gtbl_cfr_status_continent <- tbl_cfr_status_continent %>%
  gt() %>%
  cols_hide('outcome_patcourse_status') %>%
  cols_move_to_start(columns = vars(MSF_covid_status)) %>%
  cols_label(
    MSF_covid_status = 'Covid status',
    n_Africa   = 'n',
    n_Americas = 'n',
    n_Asia     = 'n',
    n_Europe   = 'n',
...
cfr_confirmed_probable_suspected <- dta_linelist %>%
  select(MSF_covid_status, outcome_patcourse_status) %>%
  filter(MSF_covid_status %in% c('Confirmed', 'Probable', 'Suspected')) %>%
  filter(outcome_patcourse_status %in% c('Cured', 'Died'), ) %>%
  summarise(cfr = mean(outcome_patcourse_status == 'Died')) %>%
  pull()
```
...
missing_age_sex <- dta_linelist %>% filter(MSF_covid_status == "Confirmed") %>% summarise(age = sum(is.na(age_in_years)), sex = sum(is.na(sex)))
...
dta_age_sex_outcome <- dta_linelist %>%
  select(site_name, MSF_covid_status, outcome_patcourse_status, sex = sex, age_in_years) %>%
  filter(MSF_covid_status %in% c('Confirmed', 'Probable', 'Suspected'))

tbl_age_sex_outcome <- dta_age_sex_outcome %>%
  group_by(outcome_patcourse_status) %>%
  summarise(
    age_median = median(age_in_years, na.rm = TRUE),
...
gtbl_age_sex_outcome <- tbl_age_sex_outcome %>%
  gt() %>%
  cols_label(
    outcome_patcourse_status = 'Status',
    age_median = 'Median',
    age__prcl = '[25% - 75%]',
    age_range = '[min - max]',
    sex_males = 'Males',
  ...
  cols_align(
    align = 'left',
    columns = vars(outcome_patcourse_status)) %>%
  cols_align(
    align = 'right',
    columns = starts_with('sex_')) %>%
  tab_options(
...
dta_outcome_admission <- dta_linelist %>%
  select(MSF_covid_status, merge_admit, tidyselect::vars_select(names(dta_linelist), starts_with('outcome_')))

tbl_outcome_admission <- dta_outcome_admission %>%
  filter(MSF_covid_status %in% c('Confirmed', 'Probable', 'Suspected')) %>%
  count(merge_admit, outcome_patcourse_status, .drop = FALSE) %>%
...
gtbl_outcome_admission <- tbl_outcome_admission %>%
  ...
  text_transform(
    locations = cells_body(
      columns = starts_with('prct_'),
      rows = outcome_patcourse_status == 'Total'),
    fn = function(x) {
      x = '---'}) %>%
  cols_align(
    align = 'right',
  ...
  cols_width(
    vars(outcome_patcourse_status) ~ px(210),
...
dta_cfr_status_hospi_icu <- dta_linelist %>%
  ...
  filter(outcome_patcourse_status %in% c('Cured', 'Died')) %>%
  # filter(merge_admit != "(Unknown)") %>%
  select(MSF_covid_status, merge_admit, outcome_patcourse_status, merge_icu, merge_oxygen,
         merge_vent) %>%
  mutate(merge_admit = recode(merge_admit, "Yes" = "Admitted", "No" = "Not Admitted"))
...
dta_cfr_status_hospi <- unique(dta_cfr_status_hospi_icu$merge_admit) %>%
  map_df(~{
    dta_cfr_status_hospi_icu %>%
      ...
      tbl_prop('outcome_patcourse_status', 'MSF_covid_status', drop_levels = FALSE) %>%
      mutate(
        merge_admit = .x)
  })  %>%
  filter(outcome_patcourse_status == 'Died')
...
gtbl_cfr_status_hospi_icu_oxy_venti <- tbl_cfr_status_hospi_icu_oxy %>%
  gt(rowname_col = "var",
     groupname_col = "label_var") %>%
  cols_hide(
    columns = vars('outcome_patcourse_status', 'n_Not a case', 'n_(Unknown)',
...
dta_cfr_comcond <- prepare_msf_dta_comcond(dta_linelist) %>%
  mutate(
    Comcond_other = case_when(
      ...
      TRUE ~ NA_character_)) %>%
  filter(MSF_covid_status == 'Confirmed', outcome_patcourse_status %in% c('Cured', 'Died')) %>%
  pivot_longer(-c(MSF_covid_status, outcome_patcourse_status, continent, ind_Comcond_count, ind_Comcond_01), names_to = "type_comorbidity") %>%
  mutate(
    type_comorbidity = factor(type_comorbidity, levels = df_labels_comcond$levels, labels = df_labels_comcond$labels)) %>%
...
df_MSF_severity <- dta_linelist %>%
  ...
  filter(!is.na(age_group), MSF_covid_status == "Confirmed") %>%
  group_by(age_group) %>%
  summarize(
    n = n(),
...
tbl_cfr_epiweek_prop <- dta_linelist %>%
  filter(
    MSF_covid_status %in% c("Confirmed", "Suspected", "Probable"),
    outcome_patcourse_status %in% c("Cured", "Died"),
    !epi_week_consultation %in% latest_2_epiweeks
  ) %>%
  drop_na(epi_week_consultation) %>%
  group_by(epi_week_consultation) %>%
  summarise(
    total = sum(!is.na(outcome_patcourse_status)),
    dead = sum(outcome_patcourse_status == "Died", na.rm = T),
...
tbl_cfr_epiweek <- dta_linelist %>%
  ...
  count(epi_week_consultation, outcome_patcourse_status) %>%
  drop_na() %>%
  ungroup()
...
line_cfr_week <- ggplot(tbl_cfr_epiweek, aes(epi_week_consultation, n)) +
  geom_col(aes(fill = outcome_patcourse_status)) +
...
tbl_cfr_epiweek_continent <- dta_linelist %>%
  ...
  count(continent, epi_week_consultation, outcome_patcourse_status) %>%
  drop_na()
...
dta_delay <- dta_linelist %>%
  filter(MSF_covid_status %in% c("Confirmed")) %>%
  select(epi_week_consultation, patcourse_dateonset, MSF_date_consultation, MSF_covid_status) %>%
  drop_na(epi_week_consultation) %>%
  mutate(
    delay_before_consultation = as.numeric(MSF_date_consultation - patcourse_dateonset)) %>%
...
dta_length_stay <- dta_linelist %>%
  select(epi_week_consultation, MSF_covid_status, merge_admit, length_stay) %>%
  filter(MSF_covid_status %in% c('Confirmed'), merge_admit == 'Yes') %>%
...
dta_non_cases_diagt <- dta_linelist %>%
  ...
  select(main_diagnosis, outcome_patcourse_status) %>%
  mutate(
    main_diagnosis = fct_explicit_na(main_diagnosis, na_level = "Not reported"))

tbl_non_cases_diagt <- tbl_prop(dta_non_cases_diagt, 'main_diagnosis', 'outcome_patcourse_status', prop_by = 'row')
...
fig4 <- dta_linelist_project %>%
  ...
  # select(country, project, merge_admit, MSF_severity, length_stay) %>%
...
tble2 <- dta_linelist_project %>%
  select(country, project, admit, merge_admit, MSF_severity, length_stay) %>%
  group_by(country, project) %>%
  summarise(n_patients = n(),
...
tble3 <- dta_linelist_project %>%
  ...
  summarise(
    ...
    q_mild_25 = quantile(length_stay[merge_admit == "Yes" & MSF_severity == "Mild"], c(0.25), na.rm = TRUE),
    q_mild_50 = quantile(length_stay[merge_admit == "Yes" & MSF_severity == "Mild"], c(0.5), na.rm = TRUE),
    q_mild_75 = quantile(length_stay[merge_admit == "Yes" & MSF_severity == "Mild"], c(0.75), na.rm = TRUE),
    
    q_moderate_25 = quantile(length_stay[merge_admit == "Yes" & MSF_severity == "Moderate"], c(0.25), na.rm = TRUE),
    q_moderate_50 = quantile(length_stay[merge_admit == "Yes" & MSF_severity == "Moderate"], c(0.5), na.rm = TRUE),
    q_moderate_75 = quantile(length_stay[merge_admit == "Yes" & MSF_severity == "Moderate"], c(0.75), na.rm = TRUE),
    
    q_critical_25 = quantile(length_stay[merge_admit == "Yes" & MSF_severity == "Critical"], c(0.25), na.rm = TRUE),
    q_critical_50 = quantile(length_stay[merge_admit == "Yes" & MSF_severity == "Critical"], c(0.5), na.rm = TRUE),
    q_critical_75 = quantile(length_stay[merge_admit == "Yes" & MSF_severity == "Critical"], c(0.75), na.rm = TRUE),
    
    q_severe_25 = quantile(length_stay[merge_admit == "Yes" & MSF_severity == "Severe"], c(0.25), na.rm = TRUE),
    q_severe_50 = quantile(length_stay[merge_admit == "Yes" & MSF_severity == "Severe"], c(0.5), na.rm = TRUE),
    q_severe_75 = quantile(length_stay[merge_admit == "Yes" & MSF_severity == "Severe"], c(0.75), na.rm = TRUE),
    
    na_MSF_severity = sum(is.na(MSF_severity)),
    na_length_of_stay = sum(is.na(length_stay)),
    na_both = sum(is.na(MSF_severity) & is.na(length_stay))
  ) %>%
...
tble6 <- dta_linelist_project %>%
  select(country, project, MSF_covid_status, merge_admit, sex, age_in_years) %>%
  group_by(country, project, sex) %>%
  summarise(
    n_patients = n(),
    n_admis    = sum(merge_admit == "Yes", na.rm = TRUE),
    
    n_probable_admis    = sum(MSF_covid_status == "Probable" & merge_admit == "Yes", na.rm = TRUE),
    q_probable_25_admis = quantile(age_in_years[MSF_covid_status == "Probable" & merge_admit == "Yes"], c(0.25), na.rm = TRUE),
    q_probable_05_admis = quantile(age_in_years[MSF_covid_status == "Probable" & merge_admit == "Yes"], c(0.5), na.rm = TRUE),
    q_probable_75_admis = quantile(age_in_years[MSF_covid_status == "Probable" & merge_admit == "Yes"], c(0.75), na.rm = TRUE),
    f_probable_admis    = format_med(q_probable_05_admis, q_probable_25_admis, q_probable_75_admis),
    
    n_confirmed_admis    = sum(MSF_covid_status == "Confirmed" & merge_admit == "Yes", na.rm = TRUE),
    q_confirmed_25_admis = quantile(age_in_years[MSF_covid_status == "Confirmed" & merge_admit == "Yes"], c(0.25), na.rm = TRUE),
    q_confirmed_05_admis = quantile(age_in_years[MSF_covid_status == "Confirmed" & merge_admit == "Yes"], c(0.5), na.rm = TRUE),
    q_confirmed_75_admis = quantile(age_in_years[MSF_covid_status == "Confirmed" & merge_admit == "Yes"], c(0.75), na.rm = TRUE),
    f_confirmed_admis = format_med(q_confirmed_05_admis, q_confirmed_25_admis, q_confirmed_75_admis),
    
    n_probable_out    = sum(MSF_covid_status == "Probable" & merge_admit == "No", na.rm = TRUE),
    q_probable_25_out = quantile(age_in_years[MSF_covid_status == "Probable" & merge_admit == "No"], c(0.25), na.rm = TRUE),
    q_probable_05_out = quantile(age_in_years[MSF_covid_status == "Probable" & merge_admit == "No"], c(0.5), na.rm = TRUE),
    q_probable_75_out = quantile(age_in_years[MSF_covid_status == "Probable" & merge_admit == "No"], c(0.75), na.rm = TRUE),
    f_probable_out    = format_med(q_probable_05_out, q_probable_25_out, q_probable_75_out),
    
    n_confirmed_out    = sum(MSF_covid_status == "Confirmed" & merge_admit == "No", na.rm = TRUE),
    q_confirmed_25_out = quantile(age_in_years[MSF_covid_status == "Confirmed" & merge_admit == "No"], c(0.25), na.rm = TRUE),
    q_confirmed_05_out = quantile(age_in_years[MSF_covid_status == "Confirmed" & merge_admit == "No"], c(0.5), na.rm = TRUE),
    q_confirmed_75_out = quantile(age_in_years[MSF_covid_status == "Confirmed" & merge_admit == "No"], c(0.75), na.rm = TRUE),
...
tble14 <- dta_linelist_project %>%
  select(country, project, merge_admit, MSF_covid_status, MSF_severity) %>%
  group_by(country, project) %>%
  summarise(
    n_suspected = sum(MSF_covid_status == "Suspected", na.rm = TRUE),
    n_probable  = sum(MSF_covid_status == "Probable", na.rm = TRUE),
    n_confirmed = sum(MSF_covid_status == "Confirmed", na.rm = TRUE),
...
tble15 <- dta_linelist_project %>%
  select(country, project, merge_admit, MSF_covid_status, MSF_severity, outcome_patcourse_status, merge_icu) %>%
  group_by(country, project) %>%
  summarise(
    n_suspected = sum(MSF_covid_status == "Suspected" & outcome_patcourse_status == "Died", na.rm = TRUE),
    n_probable  = sum(MSF_covid_status == "Probable"  & outcome_patcourse_status == "Died", na.rm = TRUE),
    n_confirmed = sum(MSF_covid_status == "Confirmed" & outcome_patcourse_status == "Died", na.rm = TRUE),
    ...
    n_deaths_icu = sum(outcome_patcourse_status == "Died" & merge_icu == "Yes", na.rm = TRUE),
...
fig7 <-dta_linelist_project %>%
  ...
  summarise(
    ...
    p_hypertension  = n_hypertension / n_hospi,
    
    n_chronic_renal = sum(Comcond_renal == "Yes", na.rm = TRUE),
    p_chronic_renal = n_chronic_renal / n_hospi) %>%
  pivot_longer(cols = c(p_diabetes, p_hypertension, p_chronic_renal),
  ...
  ggthemes::scale_colour_tableau(name = "Comorbidities",
                                 palette = "Tableau 20",
                                 breaks = c("diabetes", "hypertension", "chronic_renal"),
...
tble16 <- dta_linelist_project %>%
  select(country, project, merge_admit, ind_Comcond_01, Comcond_diabetes, hypertension,
         Comcond_renal, outcome_patcourse_status, obesity) %>%
  group_by(country, project) %>%
  summarise(
    ...
    n_comorb_death = sum(ind_Comcond_01 == 1 & outcome_patcourse_status == "Died", na.rm = TRUE),
    ...
    n_obesity_death = sum(obesity == "Yes" & outcome_patcourse_status == "Died", na.rm = TRUE),
    ...
    n_diabetes_death = sum(Comcond_diabetes == "Yes" & outcome_patcourse_status == "Died", na.rm = TRUE),
    ...
    n_hypertension_death  = sum(hypertension == "Yes" & outcome_patcourse_status == "Died", na.rm = TRUE),
    ...
    n_chronic_renal_death = sum(Comcond_renal == "Yes" & outcome_patcourse_status == "Died", na.rm = TRUE),
...
fig8 <- dta_linelist_project %>%
  filter(merge_admit == "Yes") %>%
  select(country, project, merge_admit, outcome_patcourse_status, malaria, hiv_status,
  ...
  summarise(
    ...
    p_hiv       = n_hiv / n_hospi,
    p_tb_active = n_tb_active / n_hospi,
    p_tb_past   = n_tb_past / n_hospi
  ) %>%
  
  pivot_longer(cols = c(p_malaria, p_hiv, p_tb_active, p_tb_past),
  ...
  ggthemes::scale_colour_tableau(name = "Coinfection",
                                 palette = "Tableau 20",
                                 breaks = c("hiv", "malaria", "tb_active", "tb_past"),
...
tble17 <-dta_linelist_project %>%
  ...
  summarise(n_coinf = sum(coinf == 1, na.rm = TRUE),
            ...
            n_coinf_death = sum(coinf == 1 & outcome_patcourse_status == "Died", na.rm = TRUE),
            ...
            n_malaria_death = sum(malaria == "Yes" & outcome_patcourse_status == "Died", na.rm = TRUE),
            ...
            n_hiv_death = sum(hiv_status == "Yes" & outcome_patcourse_status == "Died", na.rm = TRUE),
            ...
            n_tb_active_death = sum(tb_active == "Yes" & outcome_patcourse_status == "Died", na.rm = TRUE),
            ...
            n_tb_past_death = sum(tb_treatment_past == "Yes" & outcome_patcourse_status == "Died", na.rm = TRUE),
            ...
            n_malnutrition_death = sum(malnutrition == "Yes" & outcome_patcourse_status == "Died", na.rm = TRUE),
...
tble8 <- dta_linelist_project %>%
  select(country, project, MSF_covid_status, merge_admit, icu, merge_icu) %>%
  group_by(country, project) %>%
  summarise(
    n_patients = n(),
    n_admis    = sum(merge_admit == "Yes", na.rm = TRUE),
    ...
    n_probable_icu_admis  = sum(MSF_covid_status == "Probable" & merge_admit == "Yes" & icu == "Yes", na.rm = TRUE),
    ...
    n_probable_icu_later  = sum(MSF_covid_status == "Probable" & merge_admit == "Yes" & icu != "Yes" & merge_icu == "Yes", na.rm = TRUE),
    ...
    n_suspected            = sum(MSF_covid_status == "Suspected", na.rm = TRUE),
    n_suspected_admis      = sum(MSF_covid_status == "Suspected" & merge_admit == "Yes", na.rm = TRUE),
    p_suspected_admis      = n_suspected_admis / n_suspected,
    m_suspected_admis      = format_nbp(n_suspected_admis, p_suspected_admis),
    
    n_suspected_icu_admis  = sum(MSF_covid_status == "Suspected" & merge_admit == "Yes" & icu == "Yes", na.rm = TRUE),
    ...
    n_suspected_icu_later  = sum(MSF_covid_status == "Suspected" & merge_admit == "Yes" & icu != "Yes" & merge_icu == "Yes", na.rm = TRUE),
    ...
    n_confirmed_icu_admis  = sum(MSF_covid_status == "Confirmed" & merge_admit == "Yes" & icu == "Yes", na.rm = TRUE),
    ...
    n_confirmed_icu_later  = sum(MSF_covid_status == "Confirmed" & merge_admit == "Yes" & icu != "Yes" & merge_icu == "Yes", na.rm = TRUE),
...
tble10 <- dta_linelist_project %>%
  select(country, project, MSF_covid_status, merge_admit, received_oxygen, merge_oxygen) %>%
  group_by(country, project) %>%
  summarise(
    n_patients = n(),
    n_admis    = sum(merge_admit == "Yes", na.rm = TRUE),
    ...
    n_probable_oxygen_admis  = sum(MSF_covid_status == "Probable" & merge_admit == "Yes" & received_oxygen == "Yes", na.rm = TRUE),
    ...
    n_probable_oxygen_later  = sum(MSF_covid_status == "Probable" & merge_admit == "Yes" & received_oxygen != "Yes" & merge_oxygen == "Yes", na.rm = TRUE),
    ...
    n_suspected_oxygen_admis  = sum(MSF_covid_status == "Suspected" & merge_admit == "Yes" & received_oxygen == "Yes", na.rm = TRUE),
    ...
    n_suspected_oxygen_later  = sum(MSF_covid_status == "Suspected" & merge_admit == "Yes" & received_oxygen != "Yes" & merge_oxygen == "Yes", na.rm = TRUE),
    ...
    n_confirmed_oxygen_admis  = sum(MSF_covid_status == "Confirmed" & merge_admit == "Yes" & received_oxygen == "Yes", na.rm = TRUE),
    ...
    n_confirmed_oxygen_later  = sum(MSF_covid_status == "Confirmed" & merge_admit == "Yes" & received_oxygen != "Yes" & merge_oxygen == "Yes", na.rm = TRUE),
...
tble13 <- dta_linelist_project %>%
  select(country, project, MSF_covid_status, merge_admit, vent, merge_vent) %>%
  group_by(country, project) %>%
  summarise(
    n_patients = n(),
    n_admis    = sum(merge_admit == "Yes", na.rm = TRUE),
    ...
    n_probable_vent_admis  = sum(MSF_covid_status == "Probable" & merge_admit == "Yes" & vent == "Yes", na.rm = TRUE),
    ...
    n_probable_vent_later  = sum(MSF_covid_status == "Probable" & merge_admit == "Yes" & vent != "Yes" & merge_vent == "Yes", na.rm = TRUE),
    ...
    n_suspected_vent_admis  = sum(MSF_covid_status == "Suspected" & merge_admit == "Yes" & vent == "Yes", na.rm = TRUE),
    ...
    n_suspected_vent_later  = sum(MSF_covid_status == "Suspected" & merge_admit == "Yes" & vent != "Yes" & merge_vent == "Yes", na.rm = TRUE),
    ...
    n_confirmed_vent_admis  = sum(MSF_covid_status == "Confirmed" & merge_admit == "Yes" & vent == "Yes", na.rm = TRUE),
    ...
    n_confirmed_vent_later  = sum(MSF_covid_status == "Confirmed" & merge_admit == "Yes" & vent != "Yes" & merge_vent == "Yes", na.rm = TRUE),
...
fig6 <- dta_linelist_project %>%
  ...
  summarise(
    ...
    n_death_oxy = sum(merge_oxygen == "Yes" & outcome_patcourse_status == "Died", na.rm = TRUE),
    p_Oxygen    = round(100 * n_death_oxy / n_oxy, 1),
    
    n_vent        = sum(merge_vent == "Yes", na.rm = TRUE),
    n_death_vent  = sum(merge_vent == "Yes" & outcome_patcourse_status == "Died", na.rm = TRUE),
    p_Ventilated = round(100 * n_death_vent / n_vent, 1),
    
    n_icu       = sum(merge_icu == "Yes", na.rm = TRUE),
    n_death_icu = sum(merge_icu == "Yes" & outcome_patcourse_status == "Died", na.rm = TRUE),
...
tbl_plot1 <- dta_linelist_with_aggregated %>%
  ...
  count(country_project, MSF_covid_status, epi_week_consultation) %>%
  drop_na(epi_week_consultation) %>% 
