...
## Table 1 – Number of patient visits and hospitalisation status at admission and over course of disease, by project (includes confirmed, suspect, probable cases only)
...
tble1 <- dta_linelist %>%
  ...
  summarise(n_patients           = n(),
            ...
            n_admited_post       = sum(outcome_admit == "Yes" & admit == "No",
                                       na.rm = TRUE),
...
gtble1 <- tble1 %>% 
  ...
  tab_spanner(
    label = 'Hospitalised on admission',
  ...
  tab_spanner(
    label = 'Hospitalised (at any time)',
...
## Figure 3 – Median [IQR] length of stay, per project, stratified by severity
TODO
...
## Table 5 – Total number of patient visits by case status, ICU admission status, by project (includes confirmed, suspect, probable cases only)
...
tble5 <- dta_linelist %>%
  ...
  summarise(
    ...
    p_probable_admis      =  n_probable_admis / n_probable,
     m_probable_admis         = glue("{n_probable_admis} ({round(100 * p_probable_admis, 1)}%)"),
    ...
    p_probable_icu_admis  = n_probable_icu_admis / n_probable_admis,
     m_probable_icu_admis  = glue("{n_probable_icu_admis} ({round(100 * p_probable_icu_admis, 1)}%)"),
    n_probable_icu_later  = sum(covid_status == "Probable" & merge_admit == "Yes" & icu == "No" & merge_icu == "Yes", na.rm = TRUE),
    p_probable_icu_later  = n_probable_icu_later / n_probable_admis,
      m_probable_icu_later  = glue("{n_probable_icu_later} ({round(100 * p_probable_icu_later, 1)}%)"),
    na_merge_icu_probable = sum(is.na(merge_icu)),
    ...
    m_suspected_admis      = glue("{n_suspected_admis} ({round(100 * p_suspected_admis, 1)}%)"),
    ...
    p_suspected_icu_admis  = n_suspected_icu_admis / n_suspected_admis,
      m_suspected_icu_admis  = glue("{n_suspected_icu_admis} ({round(100 * p_suspected_icu_admis, 1)}%)"),
    n_suspected_icu_later  = sum(covid_status == "Suspected" & merge_admit == "Yes" & icu == "No" & merge_icu == "Yes", na.rm = TRUE),
    p_suspected_icu_later  = n_suspected_icu_later / n_suspected_admis,
    m_suspected_icu_later  = glue("{n_suspected_icu_later} ({round(100 * p_suspected_icu_later, 1)}%)"),
    na_merge_icu_suspected = sum(is.na(merge_icu)),
    ...
    m_confirmed_admis      = glue("{n_confirmed_admis} ({round(100 * p_confirmed_admis, 1)}%)"),
    ...
    p_confirmed_icu_admis  = n_confirmed_icu_admis / n_confirmed_admis,
     m_confirmed_icu_admis  = glue("{n_confirmed_icu_admis} ({round(100 * p_confirmed_icu_admis, 1)}%)"),
    n_confirmed_icu_later  = sum(covid_status == "Confirmed" & merge_admit == "Yes" & icu == "No" & merge_icu == "Yes", na.rm = TRUE),
    p_confirmed_icu_later  = n_confirmed_icu_later / n_confirmed_admis,
     m_confirmed_icu_later  = glue("{n_confirmed_icu_later} ({round(100 * p_confirmed_icu_later, 1)}%)"),
    na_merge_icu_confirmed = sum(is.na(merge_icu))
  ) %>% 
  select(country, project, n_patients,
         # n_admis, 
         n_probable,  m_probable_admis,  m_probable_icu_admis,  m_probable_icu_later,  na_merge_icu_probable,
         n_suspected, m_suspected_admis, m_suspected_icu_admis, m_suspected_icu_later, na_merge_icu_suspected,
         n_confirmed, m_confirmed_admis, m_confirmed_icu_admis, m_confirmed_icu_later, na_merge_icu_confirmed)
...
gtble5 <-  tble5 %>% 
  ...
  cols_label(
    ...
    n_probable            = 'Visits',
    m_probable_admis      = 'Admissions',
    m_probable_icu_admis  = 'ICU at admission',
    m_probable_icu_later  = 'ICU after admission',
    na_merge_icu_probable = 'ICU data non available',
    
    n_suspected            = 'Visits',
    m_suspected_admis      = 'Admissions',
    m_suspected_icu_admis  = 'ICU at admission',
    m_suspected_icu_later  = 'ICU after admission',
    na_merge_icu_suspected = 'ICU data non available',
    
    n_confirmed            = 'Visits',
    m_confirmed_admis      = 'Admissions',
    m_confirmed_icu_admis  = 'ICU at admission',
    m_confirmed_icu_later  = 'ICU after admission',
    na_merge_icu_confirmed = 'ICU data non available') %>% 
