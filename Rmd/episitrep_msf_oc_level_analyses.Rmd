...
hist_epicurve_consult_admit_continent <- tbl_epicurve_consultation_continent %>%
  ...
  geom_line(data = tbl_admit_prop_continent,
            aes(y = admit_prop / scaling_factor, 
                colour = "Admitted %"),
...
pyramid_age_sex_all_continent <- ggplot(tbl_pyramid_continent, aes(x = age_9gp, y = n, fill = patinfo_sex)) +
  ...
  geom_text(aes(label = abs(n), 
                hjust = if_else(n >= 0, 1.1, -0.1)), colour = "white", size = 3.5) +
  coord_flip() +
...
pyramid_age_sex_confirmed <- ggplot(tbl_pyramid_confirmed, aes(x = age_9gp, y = n, fill = patinfo_sex)) +
  ...
  scale_y_continuous(limits = pyramid_limits, 
                     breaks = pyramid_brks, 
                     label = abs) +
  theme(legend.position = "top") +
  labs(x = "Age Group", 
       y = "Confirmed Patients", 
...
dots_cfr_agegroup <- ggplot(df_MSF_severity, aes(x = age_group)) +
  ...
  geom_text(aes(label = died_lab, 
                y = p_died_upp95 + 0.02), size = 4.2) +
...
line_cfr_week <- ggplot(tbl_cfr_epiweek, aes(epi_week_consultation, n)) +
  ...
  scale_x_date(name = "Week of Admission", 
                date_breaks = x_date_labels,
               labels = scales::label_date_short(),
               # date_labels = "%V", sec.axis = ggplot2::sec_axis(trans = ~ .),
...
tbl_plot1 <- dta_linelist_with_aggregated %>%
  mutate(project2 = stringr::str_remove(project,
           ...
           stringr::str_replace_all("-", "") %>% 
           stringr::str_replace_all("/", "_") %>% 
           stringr::str_remove(pattern = "_\\|_Afrin_\\u0645\\u0631\\u0643\\u0632_\\u0639\\u0641\\u0631\\u064a\\u0646")) %>%   
