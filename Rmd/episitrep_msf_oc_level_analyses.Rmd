...
plot_epicurve_project <- function(data,
  ...
  fig_data <- data %>%
    ...
    facet_wrap(~ site_name2, ncol = ncols_facet, scales = "free_y") +
    #
    ggthemes::scale_fill_tableau(name = "Status", palette = "Tableau 20") +
    theme_light() +
    theme(legend.position = 'top',
...
tbl_epicurve_status_p <- dta_linelist_with_aggregated %>%
  mutate(site_name2 = stringr::str_remove(site_name,
                                          pattern = " \\(\\*\\)") %>%
                      stringr::str_replace_all(" ", "_") %>%
                      stringr::str_to_lower()
         ) %>%
  count(site_name2, covid_status, epi_week_consultation) %>%
  group_by(site_name2) %>%
  nest() %>%
...
fig8 <- dta_linelist_project %>%
  ...
  labs(x = "",
       y = "Percentage of hospitalised patients",
       title = "Mortality and coinfections")
...
tbl_mortality_severity_p <- dta_linelist_project %>%
...
tbl_mortality_admission_p <- dta_linelist_project %>%
  filter(country %in% c("Afghanistan", "Bangladesh", "Haiti", "Iraq", "Yemen"),
         merge_admit == "Yes") %>%
  drop_na(epi_week_admission) %>%
  group_by(country, project, epi_week_admission) %>%
  summarise(n_hospi = n(),
  ...
  mutate(plot = map2(.x = data,
                     .y = project,
                     ~ plot_mortality_admission_project(data = .x,
                                                        select_project = .y,
                                                        path_save = path.local.msf.graphs.oc.per.project)))

# print(tbl_mortality_admission_p$plot)
```
