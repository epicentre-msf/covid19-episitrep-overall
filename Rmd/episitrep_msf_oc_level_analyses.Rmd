...
gtbl_deaths_hospi_icu <- tbl_deaths_hospi_icu %>% 
  ...
  tab_spanner(
    label = 'Deaths',
    columns = vars(n_death, n_death_hospi, p_death_hospi, n_death_icu, p_death_icu)) %>%
  tab_spanner(
    label = 'Hospitalized',
    columns = ends_with('_admited')) %>%
...
gtsave(gtbl_deaths_hospi_icu, 
       file.path(path.local.msf.tables.oc, paste0('gtbl_deaths_hospi_icu', '_', week_report, '.html')), 
       inline_css = TRUE) %>% 
  invisible()
...
gtbl_deaths_hospi_icu %>% 
  tab_header(
    title = 'Number of deaths, proportion in hospital and ICU')
```
