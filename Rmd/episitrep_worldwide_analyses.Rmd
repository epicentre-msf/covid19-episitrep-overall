...
<!-- Map World cases -->
```{r MapWorldCasesCount, fig.cap = glue('Mapping of number of Covid-19 cases as of {format(max(df_ecdc$date, na.rm = TRUE), "%d %B %Y")}')}
...
tbl_cases_trend_30d <- tbl_cases_count_30d %>% 
  select(-brks) %>% 
  left_join(rename(model_cnt_cases_linear_30d[[3]] , all_of(attach_prefix(coeff_vars, 'l_cnt_30d_')))) %>% 
  mutate(
...
# It is meant to appreciated differences among models, but not to be distributed
...
tbl_deaths_trend_30d <- tbl_deaths_count_30d %>% 
  select(-brks) %>% 
  left_join(rename(model_cnt_deaths_linear_30d[[3]] , all_of(attach_prefix(coeff_vars, 'l_cnt_30d_')))) %>% 
  mutate(
    trend_linear_30d = case_when(
      l_cnt_30d_lwr > 0 ~ "Increasing", 
      l_cnt_30d_upr < 0 ~ "Declining", 
