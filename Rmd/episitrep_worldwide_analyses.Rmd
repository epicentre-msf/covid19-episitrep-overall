...
update_world_data_anyways <- FALSE
update_models_anyways <- FALSE


# Option to update ECDC data
get_updated_world_data <- ifelse(isTRUE(update_world_data_anyways) | 
...
labels <- label_breaks(breaks, big_numbers = TRUE) %>% gsub("\\<0-1\\>", "0", .)
...
**Countries with more than 1M cases** (in decreasing order): `r tbl_cases_count %>% call_countries_with_more(1000000, 'cases') %>% combine_words()`. 

**African countries with more than 10,000 cases** (in decreasing order): `r tbl_cases_count %>% filter(continent =='Africa') %>% call_countries_with_more(10000, 'cases') %>% combine_words()`. 

**African countries with more than 1000 cases** (in decreasing order): `r tbl_cases_count %>% filter(continent =='Africa') %>% call_countries_with_more(1000, 'cases') %>% combine_words()`. 
...
**Countries with increasing trend in cases** (Figure \@ref(fig:MapWorldCasesTrends)):
...
**Countries with more than 100k deaths** (in decreasing order): `r tbl_deaths_count %>% call_countries_with_more(100000, 'deaths') %>% combine_words()` (Figure \@ref(fig:MapWorldDeathsCount)).

**Countries with more than 10k deaths** (in decreasing order): `r tbl_deaths_count %>% call_countries_with_more(10000, 'deaths') %>% combine_words()`.

**African countries with less than 50 deaths** (in decreasing order): `r tbl_deaths_count %>% filter(continent == 'Africa') %>% call_countries_with_less(50, 'deaths') %>% combine_words()`.
...
**Countries with increasing trend in deaths** (Figure \@ref(fig:MapWorldDeathsTrends)):
...
The cumulative number of cases are mapped in Figure \@ref(fig:MapWorldCasesAttackRates). 

The cumulative number of deaths are mapped in Figure \@ref(fig:MapWorldDeathsAttackRates). 


<!-- Map of cumulative incidences of cases -->
```{r MapWorldCasesAttackRates, fig.cap = glue('Cumulative incidence of Covid-19 reported cases since beginning of epidemic, per 100,000 population, data until {format(date_max_report, "%d %B %Y")}')}
...
```{r MapWorldDeathsAttackRates, fig.cap = glue('Cumulative incidence of Covid-19 associated since beginning of epidemic, per 100,000 population, data until {format(date_max_report, "%d %B %Y")}')}
...
<!-- Map doubling deaths -->
```{r MapWorldDeathsDoubling, fig.cap = glue('Doubling time of the number of Covid-19 associated deaths estimated in the last 12 days (only countries with increasing trends are displayed)')}
...
```{r DotsTestsProportion, fig.cap = glue('Relation between the proportion of positive tests in the last 12 days and the cumulative number of cases reported')}
...
tbl_tests_country_non_reliable <- tbl_tests_inc %>% 
  filter(reliable == 'No') %>% 
  arrange(country) %>% 
  select(`Not reliable data` = country)

tbl_tests_country_no_data <- tbl_tests_inc %>% 
  filter(is.na(reliable)) %>% 
  arrange(country) %>% 
  select(`Not data` = country)

#tbl_country_missing_tests <- grid.arrange(dots_prop_tests,
#                                          tableGrob(tbl_tests_country_non_reliable, rows = NULL), 
#                                          tableGrob(tbl_tests_country_no_data, rows = NULL), 
#                                          nrow = 1)


#ggplot(file.path(path.local.worldwide.graphs, paste0('dots_prop_tests', 'tbl_country_missing_tests', '_', week_report, '.png')), 
#       plot = tbl_country_missing_tests, 
#       width = 8, 
#       height = 6, 
#       scale = 1, 
#       dpi = 320)

# To fix the size of the table
