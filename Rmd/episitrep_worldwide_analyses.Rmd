...
```{r MapWorldCasesTrends, layout = "l-body-outset", fig.width = 10, fig.height = 3, fig.cap = glue('Mapping of cases counts and trends during the period from {format(date_max_report - (period_trends - 1), "%d %B %Y")} to {format(date_max_report, "%d %B %Y")} ({period_trends} days)')}
...
map_world_cases_count_trend_12d_grid <- grid.arrange(map_world_cases_count_12d + labs(caption = ''), 
                                                     map_world_cases_trends, 
                                                     ncol = 2, 
                                                     respect = FALSE)

ggsave(file.path(path.local.worldwide.graphs, paste0('map_world_cases_count_trend_12d_grid', '_', week_report, '.png')), 
       plot = map_world_cases_count_trend_12d_grid, 
       scale = 1.6, 
       width = 7, 
       height = 3)
...
tbl_countries_increasing_cases <- tbl_cases_trends %>% 
  ...
  left_join(model_cnt_cases_linear_30d[[3]] %>% select(iso_a3, coeff_30d = coeff, lwr_30d = lwr, upr_30d = upr), by = 'iso_a3') %>%
  filter(trend == 'Increasing') %>% 
  ...
  mutate(
    ...
    lwr_30d = NULL,
    upr_30d = NULL) %>% 
  select(continent, country, cases, cases_30d, cases_12d, coeff_30d, coeff_ci_30d, coeff, coeff_ci)
...
```{r MapWorldDeathsTrends, layout = "l-body-outset", fig.width = 10, fig.height = 3, fig.cap = glue('Mapping of Covid-19 associated deaths and trends during the period from {format(date_max_report - (period_trends - 1), "%d %B %Y")} to {format(date_max_report, "%d %B %Y")} ({period_trends} days)')}
...
map_world_deaths_count_trend_12d_grid <- grid.arrange(map_world_deaths_count_12d + labs(caption = ''), 
                                                     map_world_deaths_trends, 
                                                     ncol = 2, 
                                                     respect = FALSE)

ggsave(file.path(path.local.worldwide.graphs, paste0('map_world_deaths_count_trend_12d_grid', '_', week_report, '.png')), 
       plot = map_world_deaths_count_trend_12d_grid, 
       scale = 1.6, 
       width = 7, 
       height = 3)
