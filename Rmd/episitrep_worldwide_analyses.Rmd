...
last_update_dta_ecdc <- rds_ecdc[[2]]
...
trend_models <- get_trend_models(periods_trends = c(12, 30))
...
model_cml_cases_linear_short <- trend_models$model_cml_cases_linear_short
...
model_cml_deaths_linear_short <- trend_models$model_cml_deaths_linear_short

lst_coeffs_cases  <- trend_models$lst_coeffs_cases
lst_coeffs_deaths <- trend_models$lst_coeffs_deaths

period_trends <- trend_models$periods_trends[[1]]
...
tbl_cases_count <- dta_ecdc_right_censored %>% 
...
tbl_cases_count_30d <- dta_ecdc_right_censored %>% 
...
tbl_cases_count_12d <- dta_ecdc_right_censored %>% 
...
tbl_deaths_count <- dta_ecdc_right_censored %>% 
  group_by(iso_a3, continent, region, country) %>% 
...
tbl_deaths_count_30d <- dta_ecdc_right_censored %>% 
  filter(between(date, left = date_max_report-29, right = date_max_report)) %>% 
  group_by(iso_a3, continent, region, country) %>% 
  summarise(deaths = sum(deaths, na.rm = TRUE)) %>% 
  mutate(brks = cut(deaths, breaks_deaths, labels = labels_deaths, include.lowest = TRUE, right = FALSE)) %>% 
...
tbl_deaths_count_12d <- dta_ecdc_right_censored %>% 
  filter(between(date, left = date_max_report-11, right = date_max_report)) %>% 
  group_by(iso_a3, continent, region, country) %>% 
  summarise(deaths = sum(deaths, na.rm = TRUE)) %>% 
  mutate(brks = cut(deaths, breaks_deaths, labels = labels_deaths, include.lowest = TRUE, right = FALSE)) %>% 
...
tbl_cases_trend <- tbl_cases_count %>% 
  select(-brks) %>% 
  left_join(rename(model_cnt_cases_linear_short[[3]] , all_of(attach_prefix(coeff_vars, 'l_cnt_')))) %>% 
  left_join(rename(model_cml_cases_linear_short[[3]] , all_of(attach_prefix(coeff_vars, 'l_cml_')))) %>% 
  mutate(
...
write.csv(tbl_cases_trend, 
          file.path(path.local.worldwide.tables, paste0('tbl_cases_trend', '_', week_report, '.csv')), 
          na = "", 
          row.names = FALSE, 
          fileEncoding = "UTF-8")
...
tbl_cases_increasing_trend <- tbl_cases_trend %>% 
...
```{r map-world-cases-trends,fig.width=8, fig.height=10, fig.cap = glue('Mapping of cases counts and trends during the period from {format(date_max_report - (period_trends - 1), "%d %B %Y")} to {format(date_max_report, "%d %B %Y")} ({period_trends} days)')}

map_world_cases_count_12d <- plot_map_world_count(tbl_cases_count_12d, 'cases') + 
  labs(title = 'Cases counts (12 days)')

ggsave(file.path(path.local.worldwide.graphs, paste0('map_world_cases_count_12d', '_', week_report, '.png')), 
       plot = map_world_cases_count_12d, 
       scale = 1, 
       dpi = 320)


map_world_cases_trends <- plot_map_world_trend(tbl_cases_trend, 'cases') + 
  labs(title = 'Cases trends (12 days)')
...
ggarrange(map_world_cases_count_12d + labs(title = '', caption = ''), 
          map_world_cases_trends  + labs(title = ''), 
          labels = c("Cases count (12 days)", "Cases trends (12 days)"), 
          ncol = 1)
...
map_world_cases_count_trend_grid <- ggarrange(
  map_world_cases_trends  + labs(title = '', caption = ''),  
  ggarrange(
    map_world_cases_count + labs(title = '', caption = ''),
    map_world_cases_count_12d  + labs(title = '', caption = ''), 
    labels = c("Cases counts (All)", "Cases counts (12 days)"), 
    common.legend = TRUE, 
    legend = 'bottom', 
    ncol = 2), 
  labels = c("Cases trends (12 days)"),
  nrow = 2) %>% 
  annotate_figure(bottom = text_grob(caption_world_map, hjust = 1, x = 1, size = 10))

ggsave(file.path(path.local.worldwide.graphs, paste0('map_world_cases_count_trend_grid', '_', week_report, '.png')), 
       plot = map_world_cases_count_trend_grid, 
       scale = 1.1, 
       width = 9, 
       height = 8, 
       dpi = 320)
...
<!-- Table cases trends -->
```{r tbl-countries-cases-increasing, layout="l-body-outset"}

tbl_countries_increasing_cases <- tbl_cases_trend %>% 
  select(c(iso_a3 : cases), all_of(vars_trends(model_for_trends))) %>% 
  ...
  left_join(tbl_cases_trend_30d %>% select(iso_a3, coeff_30d = l_cnt_30d_coeff, lwr_30d = l_cnt_30d_lwr, upr_30d = l_cnt_30d_upr, trend_30d = trend_linear_30d), by = 'iso_a3') %>%
...
<!-- Map deaths count -->
```{r map-world-deaths-count, fig.cap = glue('Mapping of number of Covid-19 associated deaths as of {format(date_max_report, "%d %b %Y")}')}
...
tbl_deaths_trend_30d <- tbl_deaths_count_30d %>% 
  select(-brks) %>% 
...
tbl_deaths_trend <- tbl_deaths_count %>% 
  select(-brks) %>% 
  left_join(rename(model_cnt_deaths_linear_short[[3]] , all_of(attach_prefix(coeff_vars, 'l_cnt_')))) %>% 
  left_join(rename(model_cml_deaths_linear_short[[3]] , all_of(attach_prefix(coeff_vars, 'l_cml_')))) %>% 
  mutate(
    trend_linear = case_when(
      l_cnt_lwr > 0 ~ "Increasing", 
      l_cnt_upr < 0 ~ "Declining", 
...
tbl_deaths_increasing_trend <- tbl_deaths_trend %>% 
...
```{r map-world-deaths-trends, fig.width=8, fig.height=10, fig.cap = glue('Mapping of Covid-19 associated deaths and trends during the period from {format(date_max_report - (period_trends - 1), "%d %B %Y")} to {format(date_max_report, "%d %B %Y")} ({period_trends} days)')}

map_world_deaths_count_12d <- plot_map_world_count(tbl_deaths_count_12d, 'deaths') + 
  labs(title = 'Deaths count (12 days)')

ggsave(file.path(path.local.worldwide.graphs, paste0('map_world_deaths_count_12d', '_', week_report, '.png')), 
       plot = map_world_deaths_count_12d, 
       scale = 1, 
       dpi = 320)

map_world_deaths_trends <- plot_map_world_trend(tbl_deaths_trend, 'deaths') + 
  labs(title = 'Deaths trends (12 days)')
...
ggarrange(map_world_deaths_count_12d + labs(title = '', caption = ''), 
          map_world_deaths_trends  + labs(title = ''), 
          labels = c("Deaths count (12 days)", "Deaths trends (12 days)"), 
          ncol = 1)
...
map_world_deaths_count_trend_grid <- ggarrange(
  map_world_deaths_trends  + labs(title = '', caption = ''),  
  ggarrange(
    map_world_deaths_count + labs(title = '', caption = ''),
    map_world_deaths_count_12d  + labs(title = '', caption = ''), 
    labels = c("Deaths counts (All)", "Deaths counts (12 days)"), 
    common.legend = TRUE, 
    legend = 'bottom', 
    ncol = 2), 
  labels = c("Deaths trends (12 days)"),
  nrow = 2) %>% 
  annotate_figure(bottom = text_grob(caption_world_map, hjust = 1, x = 1, size = 10))

ggsave(file.path(path.local.worldwide.graphs, paste0('map_world_deaths_count_trend_grid', '_', week_report, '.png')), 
       plot = map_world_deaths_count_trend_grid, 
       scale = 1.1, 
       width = 9, 
       height = 8, 
       dpi = 320)
...
<!-- Table deaths trends -->
```{r tbl-countries-deaths-increasing, layout="l-body-outset"}

tbl_countries_increasing_deaths <- tbl_deaths_trend %>% 
  select(c(iso_a3 : deaths), all_of(vars_trends(model_for_trends))) %>% 
  left_join(tbl_deaths_count_30d %>% select(iso_a3, deaths_30d = deaths), by = 'iso_a3') %>% 
  left_join(tbl_deaths_count_12d %>% select(iso_a3, deaths_12d = deaths), by = 'iso_a3') %>% 
  left_join(tbl_deaths_trend_30d %>% select(iso_a3, coeff_30d = l_cnt_30d_coeff, lwr_30d = l_cnt_30d_lwr, upr_30d = l_cnt_30d_upr, trend_30d = trend_linear_30d), by = 'iso_a3') %>%
  filter(trend == 'Increasing' | trend_30d == 'Increasing') %>% 
  select(-c(iso_a3, region)) %>% 
  arrange(continent, desc(coeff)) %>% 
  mutate(
...
labels_ar <- label_breaks(breaks_ar) %>% gsub("\\<0-1\\>", "0", .)
...
```{r map-world-cases-cumulative-incidence, fig.cap = paste0('Cumulative incidence of Covid-19 reported cases since beginning of epidemic per 100,000 population, data until ', format(date_max_report, "%d %B %Y"))}
...
```{r map-world-deaths-cumulative-incidence, fig.cap = glue('Cumulative incidence of Covid-19 associated since beginning of epidemic, per 100,000 population, data until {format(date_max_report, "%d %B %Y")}')}
...
### Cumulative incidence versus incidence in the last 30 days

<!-- Dots plot 30 days cumulative incidence versus 30 days incidence -->
```{r cumulative-incidence-versus-incidences-30-days, fig.width = 9, fig.height = 8}
# New graph as requested by Fran
# to see how to display

tbl_inc_cml <- dta_ecdc %>% 
...
ggsave(file.path(path.local.worldwide.graphs, paste0('dotsgplot_inc_Africa_30-days', '_', week_report, '.png')), 
...
ggsave(file.path(path.local.worldwide.graphs, paste0('dotsgplot_inc_Asia_30-days', '_', week_report, '.png')), 
         plot = dotsgplot_inc_Africa + labs(title = NULL), 
         width = 20 * cm_to_in, 
         height = 20 * cm_to_in, 
         scale = 1.1, 
...
ggsave(file.path(path.local.worldwide.graphs, paste0('dotsgplot_inc_Americas_30-days', '_', week_report, '.png')), 
...
ggsave(file.path(path.local.worldwide.graphs, paste0('dotsgplot_inc_Europe_30-days', '_', week_report, '.png')), 
...
ggsave(file.path(path.local.worldwide.graphs, paste0('dotsgplot_inc_Oceania_30-days', '_', week_report, '.png')), 
...
dotsgplot_inc_Africa
dotsgplot_inc_Asia
dotsgplot_inc_Americas
dotsgplot_inc_Europe

```



### Cumulative incidence versus incidence since the first cases reported

<!-- Dots plot cumulative incidences versus incidence -->
```{r cumulative-incidence-versus-incidence, fig.width = 9, fig.height = 8}
# New graph as requested by Fran
# to see how to display
...
dotsgplot_inc <- function(tbl_inc = tbl_inc_cml, name_continent){
  ...
  dotsgplot <- ggplot(temp_tbl) + 
    ...
    labs(
      subtitle = name_continent, 
      caption = paste0("Countries with zero count: ",txt_countries_zero_count, ".\n", caption_world_map), 
      y = "Cumulative incidence of cases per 100,000 population",
      x = "Cumulative number of cases",
      color = NULL) + 
    theme_light()
...
ggsave(file.path(path.local.worldwide.graphs, paste0('dotsgplot_inc_Asia', '_', week_report, '.png')), 
         plot = dotsgplot_inc_Asia + labs(title = NULL), 
         width = 20 * cm_to_in, 
         height = 20 * cm_to_in, 
         scale = 1.1, 
         dpi = 320)
...
tbl_countries_doubling_time_cases <- tbl_cases_doubling_time %>% 
  right_join(tbl_cases_increasing_trend %>% select(iso_a3)) %>% 
...
tbl_countries_doubling_time_deaths <- tbl_deaths_doubling_time %>% 
  right_join(tbl_deaths_increasing_trend %>% select(iso_a3)) %>% 
  filter(between(est, 0, threshold_doubling_time))
...
tbl_doubling <- tbl_cases_doubling_time %>% 
  ...
  full_join(tbl_deaths_doubling_time %>% 
  filter(!is.na(country)) %>% 
  rename(deaths_est = est, deaths_lwr = lwr, deaths_upr = upr)) %>% 
  right_join(full_join(tbl_cases_increasing_trend %>% select(iso_a3, trend_cases = trend), 
                       tbl_deaths_increasing_trend %>% select(iso_a3, trend_deaths = trend)))
