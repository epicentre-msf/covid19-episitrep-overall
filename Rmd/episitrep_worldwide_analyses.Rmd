...
trend_models <- get_trend_models(lst_jhu = lst_dta_jhu, periods_trend = c(14, 30), force = FALSE)
...
tbl_case_trend <- tbl_case_count %>% 
  ...
  left_join(tbl_trend_14d %>% 
              rename(cases_model = cases), 
            by = c("iso_a3", "continent", "region", "country")) %>% 
  select(-contains("death"), -contains("doubling"))
...
tbl_case_doubling_time <- tbl_case_count %>% 
  ...
  left_join(tbl_trend_14d %>% select(iso_a3, 
                         est = doubling_est_cases, 
                         lwr = doubling_lwr_cases, 
                         upr = doubling_upr_cases), by = "iso_a3")
...
tbl_death_doubling_time <- tbl_death_count %>% 
  ...
  left_join(tbl_trend_14d %>% select(iso_a3, 
                         est = doubling_est_deaths, 
                         lwr = doubling_lwr_deaths, 
                         upr = doubling_upr_deaths), by = "iso_a3")
