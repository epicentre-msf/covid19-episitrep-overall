...
**Countries with more than 100k cases** (in decreasing order): `r tbl_cases_count %>% call_countries_with(100000, Inf, 'cases') %>% combine_words()` (Figure \@ref(fig:MapWorldCasesCount)). 

**Countries with more than 1M cases** (in decreasing order): `r tbl_cases_count %>% call_countries_with(1000000, Inf, 'cases') %>% combine_words()`. 

**African countries with more than 10k cases** (in decreasing order): `r tbl_cases_count %>% filter(continent =='Africa') %>% call_countries_with(10000, Inf, 'cases') %>% combine_words()`. 

**African countries with more than 1000 cases** (in decreasing order): `r tbl_cases_count %>% filter(continent =='Africa') %>% call_countries_with(1000, 9999, 'cases') %>% combine_words()`. 


**African countries with less than 50 cases** (in decreasing order): `r tbl_cases_count %>% filter(continent == 'Africa') %>% call_countries_with(1, 49, 'cases')`. 
...
<!-- ### Dots-plots of cumulative incidences versus incidences -->
...
for (x in unique(tbl_inc_cml$continent)) {
  
  temp_tbl_inc_cml <- tbl_inc_cml %>% filter(continent == x)
  
  # The dots plot
  dots_cases_inc_cml_30days <- ggplot(temp_tbl_inc_cml) + 
    geom_point(aes(cml_count, cml_inc, col = region)) + 
    ggrepel::geom_text_repel(
      data = temp_tbl_inc_cml,
  ...
  ggsave(file.path(path.local.worldwide.graphs, paste0('dots_cases_inc_cml_30days', '_', x, '_', week_report, '.png')), 
         plot = dots_cases_inc_cml_30days + labs(title = NULL), 
         width = 20 * cm_to_in, 
         height = 20 * cm_to_in, 
         scale = 1.3, 
         dpi = 320)
