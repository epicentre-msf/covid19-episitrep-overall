...
<!-- Map World cases -->
```{r MapWorldCasesCount, fig.cap = glue('Mapping of number of Covid-19 cases as of {format(max(dta_ecdc$date, na.rm = TRUE), "%d %B %Y")}')}
...
```{r MapWorldCasesTrends,fig.width=8, fig.height=10, fig.cap = glue('Mapping of cases counts and trends during the period from {format(date_max_report - (period_trends - 1), "%d %B %Y")} to {format(date_max_report, "%d %B %Y")} ({period_trends} days)')}
...
<!-- Table cases trends -->
```{r TblCountriesCasesIncreasing, layout="l-body-outset"}
...
```{r MapWorldDeathsTrends, fig.width=8, fig.height=10, fig.cap = glue('Mapping of Covid-19 associated deaths and trends during the period from {format(date_max_report - (period_trends - 1), "%d %B %Y")} to {format(date_max_report, "%d %B %Y")} ({period_trends} days)')}
...
<!-- Table deaths trends -->
```{r TblCountriesDeathsIncreasing, layout="l-body-outset"}
...
tbl_cases_doubling_time <- tbl_cases_count %>% 
  select(-brks) %>% 
  left_join(rename(model_cnt_cases_linear_short[['doubling_time']] , all_of(attach_prefix(doubling_vars, 'l_cnt_')))) %>% 
  left_join(rename(model_cml_cases_linear_short[['doubling_time']] , all_of(attach_prefix(doubling_vars, 'l_cml_'))))
...
tbl_deaths_doubling_time <- tbl_deaths_count %>% 
  ...
  left_join(rename(model_cnt_deaths_linear_short[['doubling_time']] , all_of(attach_prefix(doubling_vars, 'l_cnt_')))) %>% 
  left_join(rename(model_cml_deaths_linear_short[['doubling_time']] , all_of(attach_prefix(doubling_vars, 'l_cml_'))))
