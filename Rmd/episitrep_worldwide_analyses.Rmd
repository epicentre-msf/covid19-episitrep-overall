...
tbl_case_trend_new <- tbl_case_count %>% 
  select(-brks) %>% 
...
write.csv(tbl_case_trend_new, 
          file.path(path.local.worldwide.tables, paste0('tbl_case_trend_new', '_', week_report, '.csv')), 
          na = "", 
          row.names = FALSE, 
          fileEncoding = "UTF-8")

tbl_case_trend_new30_d <- tbl_case_count %>% 
  select(-brks) %>% 
...
# New trends
tbl_case_increasing_trend <- tbl_case_trend_new %>% 
...
# Note: the tbl_case_count (raw data) has the sum since the beginning
# Note: the data used for the model does not include the last two days (usually incomplete)
tbl_death_trend_new <- tbl_death_count %>% 
  select(-brks) %>% 
  left_join(tbl_trend_14d %>% 
...
write.csv(tbl_death_trend_new, 
          file.path(path.local.worldwide.tables, paste0('tbl_death_trend_new', '_', week_report, '.csv')), 
          na = "", 
          row.names = FALSE, 
          fileEncoding = "UTF-8")

tbl_death_trend_new30_d <- tbl_death_count %>% 
  select(-brks) %>% 
  left_join(tbl_trend_30d %>% 
              rename(deaths_model = deaths), 
            by = c("iso_a3", "continent", "region", "country")) %>% 
...
# New trends
tbl_death_increasing_trend <- tbl_death_trend_new %>% 
...
# TRENDS
map_world_death_trend <- plot_map_world_trend(tbl_death_trend_new, 'deaths') + 
