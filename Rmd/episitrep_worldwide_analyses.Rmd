...
palette_RdAmGn <- c('#D95F02', '#E6AB02', '#1B9E77') # Three-colours palette (Red-Amber-Green) colour-blind safe
caption_world_map <- paste0('Analysis and graphic by Epicentre | Data source: JHU CSSE as of ', format(date_max_report, '%d %b %Y'))
...
saveRDS(dta_jhu, file.path(path.local.worldwide.data, 'dta_jhu.RDS'))
...
# VERY LONG
trend_models <- get_trend_models(lst_jhu = dta_jhu_right_censored, 
                                 periods_trend = c(14, 30), 
                                 force = FALSE)

# Cases 30d and 14d: trends AND doubling time
...
# Deaths 30d and 14d: trends AND doubling time
...
# For one graph, doubling time on a sliding window of dates since the begining.
...
tbl_trend_30d <- covidutils::get_trends(dta_jhu_right_censored, 
                                        time_unit_extent = 30,
                                        omit_past_days = 0) %>% 
  # get_doubling_time() %>% 
  mutate(trend_cases = factor(trend_cases, levels = c("Increasing", "Likely increasing", 
                                                      "Stable", 
                                                      "Likely decreasing", "Decreasing")),
         trend_deaths = factor(trend_deaths, levels = c("Increasing", "Likely increasing", 
                                                        "Stable", 
                                                        "Likely decreasing", "Decreasing")))


# Get linear trends and coeffs at 14 days
tbl_trend_14d <- covidutils::get_trends(dta_jhu_right_censored, 
                                        time_unit_extent = 14,
                                        omit_past_days = 0) %>% 
  mutate(trend_cases = factor(trend_cases, levels = c("Increasing", "Likely increasing", 
                                                      "Stable", 
...
tbl_preds_all <- get_all_preds(lst_dta_jhu,
                              long_trend = 30,
                              short_trend = 14)

# Save
list(tbl_trend_14d = tbl_trend_14d,
     par_14d       = get_date_extent(dta_jhu, 
                                     time_unit_extent = 14,
                                     omit_past_days = 0), 
     
     tbl_trend_30d = tbl_trend_30d,
     par_30d       = get_date_extent(dta_jhu, 
                                     time_unit_extent = 30,
                                     omit_past_days = 0),
     
     tbl_preds_all = tbl_preds_all) %>% 
  saveRDS(., file = file.path(path.local.worldwide.data, "trends_models_new.RDS")) 
```
...
tbl_case_count <- dta_jhu_right_censored %>% 
  group_by(iso_a3, continent, region, country) %>% 
  summarise(cases = sum(cases, na.rm = TRUE)) %>% 
  mutate(brks = cut(cases, breaks_cases, labels = labels_cases, 
                    include.lowest = TRUE, right = FALSE)) %>% 
  ungroup()

# Summary table of cases in the last 30 days 
tbl_case_count_30d <- dta_jhu_right_censored %>% 
  filter(between(date, left = date_max_report - 29, 
...
# Summary table of cases in the last 14 days 
tbl_case_count_14d <- dta_jhu_right_censored %>% 
  filter(between(date, left = date_max_report - 13, 
                 right = date_max_report)) %>% 
  group_by(iso_a3, continent, region, country) %>% 
  summarise(cases = sum(cases, na.rm = TRUE)) %>% 
  mutate(brks = cut(cases, breaks_cases, labels = labels_cases, 
...
tbl_death_count <- dta_jhu_right_censored %>% 
  group_by(iso_a3, continent, region, country) %>% 
  summarise(deaths = sum(deaths, na.rm = TRUE)) %>% 
  mutate(brks = cut(deaths, breaks_deaths, labels = labels_deaths, 
                    include.lowest = TRUE, right = FALSE)) %>% 
  ungroup()

# Summary table of deaths in the last 30 days 
tbl_death_count_30d <- dta_jhu_right_censored %>% 
  filter(between(date, left = date_max_report - 29, 
                 right = date_max_report)) %>% 
  group_by(iso_a3, continent, region, country) %>% 
  summarise(deaths = sum(deaths, na.rm = TRUE)) %>% 
...
# Summary table of deaths in the last 14 days 
tbl_death_count_14d <- dta_jhu_right_censored %>% 
  filter(between(date, left = date_max_report - 13, 
                 right = date_max_report)) %>% 
  group_by(iso_a3, continent, region, country) %>% 
  summarise(deaths = sum(deaths, na.rm = TRUE)) %>% 
...
n_cases_14d  <- tbl_case_count_14d %>% select(cases) %>% sum()
n_deaths_14d <- tbl_death_count_14d %>% select(deaths) %>% sum()
...
This document uses the following data sources: 
  ...
  - [JHU CSSE data](https://raw.githubusercontent.com/owid/covid-19-data/master/public/data/jhu/full_data.csv) last updated `r format(max(dta_jhu$date, na.rm = TRUE), "%d %b %Y")`.   
...
<!-- Map World cases -->
```{r map-world-cases-count, fig.cap = glue('Mapping of number of Covid-19 cases as of {format(max(dta_jhu$date, na.rm = TRUE), "%d %B %Y")}')}
...
# 14 days
# Note: the tbl_case_count (raw data) has the sum since the beginning 
# tbl_case_trend <- tbl_case_count %>% 
#   select(-brks) %>% 
#   left_join(rename(model_cnt_cases_linear_short[[3]] ,
#                    all_of(attach_prefix(coeff_vars, 'l_cnt_')))) %>% 
#   mutate(
...
# 12 days
# Note: the tbl_case_count (raw data) has the sum since the beginning
# Note: the data used for the model does not include the last two days (usually incomplete)
tbl_case_trend <- tbl_case_count %>% 
  select(-brks) %>% 
  left_join(tbl_trend_14d %>% rename(cases_model = cases), 
            by = c("iso_a3")) %>% 
  select(-contains("death"), -contains("dt"), -contains("preds"))


write.csv(tbl_case_trend, 
          file.path(path.local.worldwide.tables, paste0('tbl_case_trend',
                                                        '_', week_report, '.csv')), 
          na = "", 
          row.names = FALSE, 
          fileEncoding = "UTF-8")

tbl_case_trend_30d <- tbl_case_count %>% 
  select(-brks) %>% 
  left_join(tbl_trend_30d %>% rename(cases_model = cases), 
            by = c("iso_a3")) %>% 
  select(-contains("death"), -contains("dt"), -contains("preds"))
...
```{r list-increasing-trends-cases, include = FALSE}
...
# New trends
tbl_case_increasing_trend <- tbl_case_trend %>% 
  filter(trend_cases  %in% c('Increasing', "Likely increasing")) %>% 
  arrange(continent, desc(trend_cases_coeff)) %>% 
  select(-contains("death"), -contains("dt")) %>% 
  select(iso_a3:cases, 
         coeff = trend_cases_coeff,
         lwr   = trend_cases_coeff_lwr95,
         upr   = trend_cases_coeff_upr95,
         trend = trend_cases)
```
...
<!-- Map cases counts and trends in the last 14 days -->
...
# COUNTS
map_world_case_count_14d <- plot_map_world_count(tbl_case_count_14d, 'cases') + 
  labs(title = 'Case count (14 days)')

ggsave(file.path(path.local.worldwide.graphs, paste0('map_world_case_count_14d', '_', week_report, '.png')), 
       plot = map_world_case_count_14d, 
       scale = 1, 
       dpi = 320)

# TRENDS
map_world_case_trend <- plot_map_world_trend(tbl_case_trend, "cases") + 
  labs(title = 'Case trend (14 days)')
...
ggarrange(map_world_case_count_14d + labs(title = '', caption = ''), 
          map_world_case_trend  + labs(title = ''), 
          labels = c("Case count (14 days)", "Case trend (14 days)"), 
          ncol = 1)
...
map_world_case_count_trend_grid <- ggarrange(
  ...
  ggarrange(
    ...
    map_world_case_count_14d  + labs(title = '', caption = ''), 
    labels = c("Cumulative case counts", "Case count (14 days)"), 
    common.legend = TRUE, 
    legend = 'bottom', 
    ncol = 2), 
  labels = c("Case trend (14 days)"),
  nrow = 2) %>% 
...
increasing <- c("Increasing", "Likely increasing")

# tbl_countries_increasing_cases <- tbl_case_trend %>% 
#   select(c(iso_a3 : cases), all_of(vars_trends(model_for_trend))) %>% 
#   left_join(tbl_case_count_30d %>% select(iso_a3, cases_30d = cases), by = 'iso_a3') %>% 
#   left_join(tbl_case_count_14d %>% select(iso_a3, cases_14d = cases), by = 'iso_a3') %>% 
#   left_join(tbl_case_trend_30d %>% select(iso_a3, coeff_30d = l_cnt_30d_coeff, 
#                                           lwr_30d = l_cnt_30d_lwr, upr_30d = l_cnt_30d_upr,
#                                           trend_30d = trend_linear_30d), by = 'iso_a3') %>%
...
#       TRUE ~ coeff_ci_30d)) %>% 
#   select(continent, country, cases, cases_30d, cases_14d, coeff_30d, coeff_ci_30d, coeff, coeff_ci, trend, trend_30d)


tbl_countries_increasing_cases <- tbl_case_trend %>% 
  select(c(iso_a3 : cases)) %>% 
  # add counts
  left_join(tbl_case_count_30d %>%  
              select(iso_a3, cases_30d = cases),
            by = 'iso_a3') %>% 
  left_join(tbl_case_count_14d %>%  
              select(iso_a3, cases_14d = cases),
            by = 'iso_a3') %>% 
  #add trends  
  left_join(tbl_trend_14d %>% 
              select(iso_a3,
                     trend_cases_14d = trend_cases, 
                     trend_cases_coef_14d = trend_cases_coeff), 
            by = 'iso_a3') %>% 
  
  left_join(tbl_trend_30d %>% 
              select(iso_a3, 
                     trend_cases_30d = trend_cases, 
                     trend_cases_coef_30d = trend_cases_coeff), 
            by = 'iso_a3') %>% 
  filter(trend_cases_14d %in% increasing | trend_cases_30d %in% increasing) %>% 
  arrange(continent, desc(trend_cases_coef_14d)) %>% 
  select(continent, country, cases, cases_30d, cases_14d, trend_cases_30d, trend_cases_14d, trend_cases_coef_14d, trend_cases_coef_30d)
...
gtbl_countries_increasing_cases <- tbl_countries_increasing_cases %>% 
  select(-c(trend_cases_coef_14d, trend_cases_coef_30d)) %>% 
  gt(rowname_col = 'country', groupname_col = 'continent') %>% 
  
  cols_label(
    cases     = 'Total', 
    cases_30d = 'Last 30 days', 
    cases_14d = 'Last 14 days',  
    trend_cases_30d = '30 days',
    trend_cases_14d = '14 days') %>% 
  tab_spanner(
    label = 'Number of cases', 
    columns = starts_with('cases')) %>% 
  tab_spanner(
    label = 'Trends', 
    columns = starts_with('trend'))  %>% 
  fmt_number(
    columns = starts_with('cases'), 
    decimals = 0) %>% 
  
  # Color based on trend category
  tab_style(
    style = list(
      cell_fill(color = "#8A331F", alpha = 0.8),
      cell_text(weight = 'bold')),  
    locations = cells_body(
      columns = vars(trend_cases_30d), 
      rows = trend_cases_30d == 'Increasing')) %>% 
  
   tab_style(
    style = list(
      cell_fill(color = "#CA845F"  , alpha = 0.8)), 
    locations = cells_body(
      columns = vars(trend_cases_30d), 
      rows = trend_cases_30d == 'Likely increasing')) %>% 
  
  tab_style(
    style = list(
      cell_fill(color = "#D4BEB4", alpha = 0.8)), 
    locations = cells_body(
      columns = vars(trend_cases_30d), 
      rows = trend_cases_30d == 'Stable')) %>% 
  
    tab_style(
    style = list(
      cell_fill(color = "#80A5C0", alpha = 0.8)), 
    locations = cells_body(
      columns = vars(trend_cases_30d), 
      rows = trend_cases_30d == 'Likely decreasing')) %>% 
  
    tab_style(
    style = list(
      cell_fill(color = "#345B8C", alpha = 0.8)), 
    locations = cells_body(
      columns = vars(trend_cases_30d), 
      rows = trend_cases_30d == 'Decreasing')) %>% 
  ...
  tab_style(
    style = list(
      ...
      cell_text(weight = 'bold')), 
    locations = cells_body(
      columns = vars(trend_cases_14d), 
      rows = trend_cases_14d == 'Increasing')) %>% 
   ...
   tab_style(
    ...
    locations = cells_body(
      columns = vars(trend_cases_14d), 
      rows = trend_cases_14d == 'Likely increasing')) %>% 
  ...
  tab_style(
   ...
   tab_style(
    ...
    locations = cells_body(
      columns = vars(trend_cases_14d), 
      rows = trend_cases_14d == 'Likely decreasing')) %>% 
    ...
    locations = cells_body(
      columns = vars(trend_cases_14d), 
      rows = trend_cases_14d == 'Decreasing')) %>% 
...
gtbl_countries_increasing_cases %>% 
  tab_header(
    title = 'Countries with a significant increasing trend in cases in the last 30 or 14 days')
...
tbl_death_trend <- tbl_death_count %>% 
  ...
  left_join(tbl_trend_14d %>% 
            ...
            by = c("iso_a3")) %>% 
  select(-contains("cases"), -contains("doubling"), -contains("preds"))
...
tbl_death_trend_30d <- tbl_death_count %>% 
  select(-brks) %>% 
  left_join(tbl_trend_30d %>% 
              rename(deaths_model = deaths), 
            by = c("iso_a3")) %>% 
...
```{r list-increasing-deaths-cases}
...
# New trends
tbl_death_increasing_trend <- tbl_death_trend %>% 
 filter(trend_deaths  %in% c('Increasing', "Likely increasing")) %>% 
  arrange(continent, desc(trend_deaths_coeff)) %>% 
  select(-contains("cases"), -contains("doubling")) %>% 
  select(iso_a3:deaths, 
         coeff = trend_deaths_coeff,
         lwr   = trend_deaths_coeff_lwr95,
         upr   = trend_deaths_coeff_upr95,
         trend = trend_deaths)
...
<!-- Map deaths counts and trends in the last 14 days -->
...
# COUNTS
map_world_death_count_14d <- plot_map_world_count(tbl_death_count_14d, 'deaths') + 
  labs(title = 'Death count (14 days)')

ggsave(file.path(path.local.worldwide.graphs, paste0('map_world_death_count_14d', '_', week_report, '.png')), 
       plot = map_world_death_count_14d, 
       scale = 1, 
       dpi = 320)
...
map_world_death_trend <- plot_map_world_trend(tbl_death_trend, 'deaths') + 
  labs(title = 'Death trend (14 days)')
...
ggarrange(map_world_death_count_14d + labs(title = '', caption = ''), 
          map_world_death_trend  + labs(title = ''), 
          labels = c("Death count (14 days)", "Death trends (14 days)"), 
          ncol = 1)
...
map_world_death_count_trend_grid <- ggarrange(
  ...
  ggarrange(
    ...
    map_world_death_count_14d  + labs(title = '', caption = ''), 
    labels = c("Cumulative death count", "Death count (14 days)"), 
    common.legend = TRUE, 
    legend = 'bottom', 
    ncol = 2), 
  labels = c("Death trend (14 days)"),
  nrow = 2) %>% 
  annotate_figure(bottom = text_grob(caption_world_map, hjust = 1, x = 1, size = 10))
...
#   left_join(tbl_death_count_14d %>% select(iso_a3, deaths_14d = deaths), by = 'iso_a3') %>% 
...
#       coeff_ci_30d == '[NA - NA]' ~ NA_character_, 
#       TRUE ~ coeff_ci_30d)) %>% 
#   select(continent, country, deaths, deaths_30d, deaths_14d, coeff_30d, coeff_ci_30d, coeff, coeff_ci, trend, trend_30d)

tbl_countries_increasing_deaths <- tbl_death_trend %>% 
  select(c(iso_a3 : deaths)) %>% 
  # add counts
  left_join(tbl_death_count_30d %>%  
              select(iso_a3, deaths_30d = deaths),
            by = 'iso_a3') %>% 
  left_join(tbl_death_count_14d %>%  
              select(iso_a3, deaths_14d = deaths),
            by = 'iso_a3') %>% 
  #add trends  
  left_join(tbl_trend_14d %>% 
              select(iso_a3,
                     trend_deaths_14d = trend_deaths, 
                     trend_deaths_coef_14d = trend_deaths_coeff), 
            by = 'iso_a3') %>% 
  
  left_join(tbl_trend_30d %>% 
              select(iso_a3, 
                     trend_deaths_30d = trend_deaths, 
                     trend_deaths_coef_30d = trend_deaths_coeff), 
            by = 'iso_a3') %>% 
  filter(trend_deaths_14d %in% increasing | trend_deaths_30d %in% increasing) %>% 
  arrange(continent, desc(trend_deaths_coef_14d)) %>% 
  select(continent, country, deaths, deaths_30d, deaths_14d, trend_deaths_30d, trend_deaths_14d, trend_deaths_coef_14d, trend_deaths_coef_30d)
...
gtbl_countries_increasing_deaths <- tbl_countries_increasing_deaths %>% 
  select(-c(trend_deaths_coef_14d, trend_deaths_coef_30d)) %>% 
  gt(rowname_col = 'country', groupname_col = 'continent') %>% 
  
  cols_label(
    deaths     = 'Total', 
    deaths_30d = 'Last 30 days', 
    deaths_14d = 'Last 14 days',  
    trend_deaths_30d = '30 days',
    trend_deaths_14d = '14 days') %>% 
  tab_spanner(
  ...
  tab_style(
    style = list(
      ...
      cell_text(weight = 'bold')),  
    locations = cells_body(
      columns = vars(trend_deaths_30d), 
      rows = trend_deaths_30d == 'Increasing')) %>% 
  ...
  tab_style(
    ...
    locations = cells_body(
      columns = vars(trend_deaths_30d), 
      rows = trend_deaths_30d == 'Likely increasing')) %>% 
  ...
  tab_style(
    ...
    locations = cells_body(
      columns = vars(trend_deaths_30d), 
      rows = trend_deaths_30d == 'Likely decreasing')) %>% 
  ...
  tab_style(
    ...
    locations = cells_body(
      columns = vars(trend_deaths_30d), 
      rows = trend_deaths_30d == 'Decreasing')) %>% 
  ...
  tab_style(
    style = list(
      ...
      cell_text(weight = 'bold')), 
    locations = cells_body(
      columns = vars(trend_deaths_14d), 
      rows = trend_deaths_14d == 'Increasing')) %>% 
  ...
  tab_style(
    ...
    locations = cells_body(
      columns = vars(trend_deaths_14d), 
      rows = trend_deaths_14d == 'Likely increasing')) %>% 
  ...
  tab_style(
    ...
    locations = cells_body(
      columns = vars(trend_deaths_14d), 
      rows = trend_deaths_14d == 'Likely decreasing')) %>% 
  ...
  tab_style(
    ...
    locations = cells_body(
      columns = vars(trend_deaths_14d), 
      rows = trend_deaths_14d == 'Decreasing')) %>% 
...
gtbl_countries_increasing_deaths %>% 
  tab_header(
    title = 'Countries with a significant increasing trend in deaths in the last 30 or 14 days')
...
tbl_summary_cases_deaths <- df_countries %>%
  ...
  mutate(
    trend_cases_coeff = round(exp(trend_cases_coeff), digits = 2),
    trend_cases_ci = case_when(
      is.na(trend_cases_coeff) ~ NA_character_,
      TRUE ~ combine_ci(exp(trend_cases_coeff_lwr95), 
                        exp(trend_cases_coeff_upr95), digits = 2))) %>%
  select(-one_of('trend_cases_coeff_lwr95', 
                 'trend_cases_coeff_upr95', 
                 'trend_cases', 
                 "cases_model")) %>%
  left_join(tbl_death_trend) %>%
  mutate(
    trend_deaths_coeff = round(exp(trend_deaths_coeff), digits = 2),
    trend_deaths_ci = case_when(
      is.na(trend_deaths_coeff) ~ NA_character_,
      TRUE ~ combine_ci(exp(trend_deaths_coeff_lwr95), 
                        exp(trend_deaths_coeff_upr95), digits = 2))) %>%
  select(-one_of('trend_deaths_coeff_lwr95', 'trend_deaths_coeff_upr95', 
                 'trend_deaths', "deaths_model")) %>%
  mutate(
  ...
  select(iso_a3, continent, region, country,
         cases, deaths, cfr,
         cases_ip, deaths_ip, cases_ar, deaths_ar,
         starts_with('trend_cases_'), starts_with('trend_deaths_')) %>%
  left_join(select(tbl_case_count_14d, iso_a3, cases_14d = cases)) %>%
  left_join(select(tbl_death_count_14d, iso_a3, deaths_14d = deaths)) %>%
  group_by(continent, region) %>%
  arrange(country, .by_group = TRUE) %>%
  ungroup()

gtbl_summary_cases_deaths <- tbl_summary_cases_deaths %>%
  select(-cases_ar, -deaths_ar) %>% 
  ...
  summary_rows(
    groups = TRUE, 
    columns = vars(cases, deaths, cases_14d, deaths_14d), 
  ...
  tab_header(title = glue('Worldwide Covid-19 cases and associated deaths, and trends as of {format(date_max_report, "%d %b %Y")}'),
             subtitle = html('<strong>Trends are estimate using data of the previous 14 days</strong><br>
...
labels_ip <- label_breaks(breaks_ip) %>% 
  gsub("\\<0-1\\>", "0", .)
...
tbl_inc_prop <- tbl_summary_cases_deaths %>% 
  select(iso_a3 : country, cases, deaths, 
         cases_ip, deaths_ip,
         cases_ar, deaths_ar) %>% 
  mutate(
    # cases_brks  = cut(cases_ip, 
    #                   breaks_ip, 
    #                   labels = labels_ip, 
    #                   include.lowest = TRUE, 
    #                   right = FALSE), 
    # deaths_brks = cut(deaths_ip,
    #                   breaks_ip, 
    #                   labels = labels_ip,
    #                   include.lowest = TRUE, 
    #                   right = FALSE),
    
     cases_brks  = cut(cases_ar, 
                       breaks_ar, 
                       labels = labels_ar, 
                       include.lowest = TRUE, 
                       right = FALSE), 
    deaths_brks = cut(deaths_ar, 
                      breaks_ar, 
                      labels = labels_ar, 
                      include.lowest = TRUE, 
                      right = FALSE))
...
map_world_death_attack_rates <- ggplot(sf_inc_prop) + 
  ...
  scale_y_continuous(expand = c(0, 0)) +
  scale_fill_brewer(
    name = "Incidence of deaths", palette = "Reds", 
    drop = FALSE, 
    guide = guide_legend(
      keyheight = unit(3, units = "mm"),
...
tbl_case_doubling_time <- tbl_case_count %>% 
  select(-brks) %>% 
  left_join(tbl_trend_14d %>% select(iso_a3, 
                         est = dt_cases_est, 
                         lwr = dt_cases_lwr, 
                         upr = dt_cases_upr), by = "iso_a3")
...
<!-- Map doubling cases -->
```{r map-world-cases-doubling, fig.cap = glue('Doubling time of the number of Covid-19 cases estimated in the last 14 days (only countries with increasing trends are displayed)')}

breaks <- c(0, 4, 8, 14, Inf)
...
df_doubling_cases <- tbl_case_doubling_time %>% 
  ...
  left_join(tbl_case_trend %>% 
              select(iso_a3, trend = trend_cases), 
            by = 'iso_a3') %>% 
  mutate(
...
df_doubling_deaths <- tbl_death_doubling_time %>% 
  ...
  left_join(tbl_death_trend %>% 
              select(iso_a3, trend = trend_deaths), 
            by = 'iso_a3') %>% 
  mutate(
    doubling_time = case_when(
      (trend != 'Increasing' | is.na(trend)) ~ NA_real_,
...
tbl_tests_inc <- df_countries %>% 
  left_join(df_pop_country %>% select(iso_a3, pop)) %>% 
  left_join(tbl_tests) %>% 
  left_join(tbl_case_count_14d %>% select(iso_a3, cases_14d = cases)) %>% 
  right_join(tbl_case_increasing_trend %>% select(iso_a3, coeff)) %>% 
  mutate(
    cases_inc_14d = cases_14d / pop * 100000, 
    tests_prop    = cases_14d / tests_cml, 
    tests_ratio   = tests_cml / cases_14d, 
...
```{r dots-tests-proportion, fig.cap = 'Relation between the proportion of positive tests in the last 14 days and the cumulative number of cases reported'}

tbl_tests_inc_reliable <- tbl_tests_inc %>% 
  filter(reliable == 'Yes')

dots_prop_tests <- ggplot(tbl_tests_inc_reliable, aes(tests_prop, cases_inc_14d)) + 
  geom_vline(xintercept = 0.05, linetype = "dashed", color = "red", size = 0.5) + 
  geom_point(aes(colour = continent)) + 
  ggrepel::geom_text_repel(
    aes(tests_prop, cases_inc_14d, label = country, colour = continent),
    size = 3,
    show.legend = FALSE) + 
  scale_x_continuous(labels = scales::percent_format(accuracy = 1)) + 
  scale_y_log10(labels = scales::label_number_si()) + 
  labs(
    y = "Cumulative number of cases per 100,000 people in the last 14 days (log scale)",
    x = glue('Proportion of Covid positive tests in the last 14 days'), 
    colour = 'Continent', 
...
<!-- Plot test ratio -->
```{r dots-tests-ratio, fig.cap = glue('Relation between the number of tests carried out in the last 14 days (as ratio of the number of confirmed cases) and the cumulative number of cases reported')}

dots_ratio_tests <- ggplot(tbl_tests_inc, aes(tests_ratio, cases_inc_14d)) + 
  geom_point(aes(tests_ratio, cases_inc_14d, colour = continent)) + 
  ...
  ggrepel::geom_text_repel(
    aes(tests_ratio, cases_inc_14d, label = country, colour = continent),
    size = 3,
    show.legend = FALSE) + 
...
<!-- Plot test rate -->
```{r dots-tests-rate, fig.cap = glue('Relation between the number of tests carried out in the last 14 days (as rate of the country population) and the cumulative number of cases reported')}

dots_rate_tests <- ggplot(tbl_tests_inc, aes(tests_rate, cases_inc_14d)) + 
  geom_point(aes(tests_rate, cases_inc_14d, colour = continent)) + 
  geom_point(data = tbl_tests_inc %>% filter(reliable == 'No'), 
             pch = 21, fill = NA, size = 4, colour = "red", stroke = 1) + 
  geom_point(data = tbl_tests_inc %>% filter(tests_cml == 0), 
             pch = 21, fill = NA, size = 4, colour = "black", stroke = 1) + 
  ggrepel::geom_text_repel(
    aes(tests_rate, cases_inc_14d, label = country, colour = continent),
    size = 3, 
    show.legend = FALSE) + 
  scale_x_continuous(trans = 'log10', 
                     breaks = scales::trans_breaks("log10", function(x) round(10^x, 0)), 
...
```{r map-deepdives, message=FALSE, results='asis'}
continent_list <- list("Africa", "Americas", "Asia", "Europe")

if(dir.exists(here(path.local.week.deepdive, "Africa"))) {
  
  cat('\n#', "Continent cases trends", '\n')
  ...
  for (i in continent_list) {
    ...
    if(file.exists(here(path.local.week.deepdive, i,
                        glue::glue("map_cases_trends_{week_report}.png")))) {
      cat(paste0("![](", 
                 here(path.local.week.deepdive, i,
                      glue::glue("map_cases_trends_{week_report}.png")), 
                 ")"), "\n") 
    }
    
    if(file.exists(here(path.local.week.deepdive, i,
                        glue::glue("preds_{week_report}.png")))) {
      cat(paste0("![](", 
                 here(path.local.week.deepdive, i,
                      glue::glue("preds_{week_report}.png")), 
                 ")"), "\n")
    ...
    # cat(paste0("![](", 
    #            here(path.local.geofacet, i, 
    #               glue::glue('{i}_geofacet_cases_free_y_{week_report}_60d.png')), 
    #            ")"), "\n")
    # 
    # cat(paste0("![](", 
    #            here(path.local.geofacet, i,
    #                 glue::glue('{i}_geofacet_cases_fixed_{week_report}_60d.png')), 
    #            ")"), "\n")
...
# Current week data
tbl_obs_increasing_trend_14d_new <- bind_rows(
  tbl_case_increasing_trend %>% 
    mutate(obs = "cases",
           week = week_report) %>% 
    rename(count = cases),
  
  tbl_death_increasing_trend %>% 
    mutate(obs = "deaths",
           week = week_report) %>% 
    rename(count = deaths)
)
...
tbl_doubling_cfr_rank_week_new <- tbl_doubling_cfr_rank %>% 
  mutate(week = week_report)


# Si les données doivent être mises à jour avec les données de la semaine:
if (max(tbl_obs_increasing_trend_14d$week) != week_report) {
  
  tbl_obs_increasing_trend_14d <- bind_rows(
    tbl_obs_increasing_trend_14d_new,
    tbl_obs_increasing_trend_14d)
  
  tbl_doubling_cfr_rank_week <- bind_rows(
    tbl_doubling_cfr_rank_week, 
    tbl_doubling_cfr_rank_week_new)
  
  saveRDS(tbl_obs_increasing_trend_14d, 
          file.path(path.sharepoint.sitrep.data, 'tbl_obs_increasing_trend_14d.RDS'))
  
  saveRDS(tbl_doubling_cfr_rank_week, 
          file.path(path.sharepoint.sitrep.data, 'tbl_doubling_cfr_rank_week.RDS'))
  
}


write.csv2(tbl_obs_increasing_trend_14d_new, 
           file.path(path.sharepoint.sitrep.data, 
                     paste0('tbl_obs_increasing_trend_14d_',
                     week_report, ".csv")))

write.csv2(tbl_doubling_cfr_rank_week_new, 
           file.path(path.sharepoint.sitrep.data, 
                     paste0('tbl_doubling_cfr_rank_week_',
                     week_report, ".csv")))
