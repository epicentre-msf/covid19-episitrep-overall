...
trend_models <- get_trend_models(periods_trend = c(12, 30))
...
write.csv(tbl_death_trend, 
          file.path(path.local.worldwide.tables, glue("tbl_death_trend_{week_report}.csv")), 
          na = "", 
          row.names = FALSE, 
          fileEncoding = "UTF-8")
...
tbl_summary_cases_deaths <- df_countries %>% 
  ...
  left_join(tbl_death_count_12d %>% select(iso_a3, deaths_12d = deaths)) %>%
...
The cumulative number of cases are mapped in Figure \@ref(fig:map-world-cases-cumulative-incidence). The cumulative number of deaths are mapped in Figure \@ref(fig:map-world-deaths-cumulative-incidence). 
...
map_world_case_attack_rates <- ggplot(sf_attack_rates) + 
...
ggsave(file.path(path.local.worldwide.graphs, glue("map_world_case_attack_rates_{week_report}.png")), 
...
map_world_death_attack_rates <- ggplot(sf_attack_rates) + 
...
ggsave(file.path(path.local.worldwide.graphs, glue("map_world_death_attack_rates_{week_report}.png")), 
