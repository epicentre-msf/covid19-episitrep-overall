...
df_countries <- readRDS(file.path(path.data, paste0('df_countries','.RDS'))) %>% 
  ...
  left_join(readRDS(file.path(path.data, paste0('df_pop_country','.RDS'))))

# sf Robinson projection
sf_robinson_Americas <- readRDS(file.path(path.data, paste0('sf_world','.RDS'))) %>% 
  filter(continent == 'Americas')
...
path.local.South_America <- file.path(path.local, 'South-America')
dir.create(path.local.South_America, showWarnings = FALSE, recursive = TRUE) 
...
for(i in names(lst_dta_ecdc)){
  mdl <- linear_trend(dta = lst_dta_ecdc[[i]], series = 'cases', n_days = 14, last_date = date_max_report)
...
for(i in names(lst_dta_ecdc)){
  mdl <- linear_trend(dta = lst_dta_ecdc[[i]], series = 'deaths', n_days = 14, last_date = date_max_report)
...
for(i in names(lst_dta_ecdc)){
  mdl <- linear_trend(dta = lst_dta_ecdc[[i]], series = 'cases', n_days = 30, last_date = date_max_report)
...
for(i in names(lst_dta_ecdc)){
  mdl <- linear_trend(dta = lst_dta_ecdc[[i]], series = 'deaths', n_days = 30, last_date = date_max_report)
