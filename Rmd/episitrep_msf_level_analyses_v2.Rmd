...
<!-- Age/sex pyramid -->
```{r PyramidAgeConfirmed, fig.cap = 'Age pyramid of patients consulted/admitted to an MSF supported Covid-19 health service', fig.width = 8, fig.height = 6}
...
p_pyramid_age_sex <- ggplot(tbl_pyramid, aes(x = age_9gp, y = n, fill = sex)) +
  geom_col() +
...
ggsave(file.path(path.local.msf.graphs, paste0('msf', 'p_pyramid_age_sex_', week_report, '.png')),
...
p_age_cfr <- ggplot(df_severity, aes(x = age_group)) +
  ...
  scale_y_continuous(breaks = seq(0, 1, 0.2), labels = scales::percent_format(accuracy = 1), sec.axis = dup_axis(name = NULL)) + # limits = c(0, 1),  
  labs(x = "Age Group", y = "CFR", caption = "CFR = deaths / known outcomes (recovered + died)\nline range shows binomial 95% confidence intervals") 
