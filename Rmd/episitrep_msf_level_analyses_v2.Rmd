...
All the required data are not available yet. And this section will be updated as soon as they are available.
...
median_age_confirmed <- dta %>% filter(covid_status == 'Confirmed') %>% pull(age_in_years) %>% median(na.rm = TRUE)
median_age_confirmed_died <- dta %>% filter(covid_status == 'Confirmed' & outcome_status == 'Died') %>% pull(age_in_years) %>% median(na.rm = TRUE)
...
```{r}
# DONE by Francesco
# Check to see if Europe needs to be added
