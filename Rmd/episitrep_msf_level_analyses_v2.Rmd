...
<!-- Age/sex pyramid -->
```{r, fig.cap = 'Age pyramid of patients consulted/admitted to an MSF supported Covid-19 health service', fig.width = 8, fig.height = 8, eval=FALSE}
# DONE by Francesco
...
```{r, fig.cap = 'Age pyramid of patients consulted/admitted to an MSF supported Covid-19 health service', fig.width = 8, fig.height = 6}
...
pyramid_df <- dta %>% 
  ...
  filter(covid_status == c("Confirmed")) %>% 
  count(sex, age_10gp) %>% 
  mutate(n = if_else(sex == "M", -n, n))

p_pyramid_age_sex <- ggplot(pyramid_df, aes(x = age_10gp, y = n, fill = sex)) +
  geom_col() +
...
```{r, fig.cap = 'Age pyramid of patients consulted/admitted to an MSF supported Covid-19 health service by continent', fig.width = 8, fig.height = 8}

m_age_sex <- dta %>% filter(covid_status == "Confirmed") %>% summarise(age = sum(is.na(age_in_years)), sex = sum(is.na(sex)))

pyramid_df_continent <- dta %>% 
  drop_na(sex, age_in_years) %>% 
  filter(covid_status %in% c("Confirmed")) %>% 
  count(continent, sex, age_10gp) %>% 
  mutate(n = if_else(sex == "M", -n, n))

p_pyramid_age_sex_continent <- ggplot(pyramid_df_continent, aes(x = age_10gp, y = n, fill = sex)) +
