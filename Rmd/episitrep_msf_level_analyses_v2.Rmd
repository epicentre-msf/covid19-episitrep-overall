...
```{r TblCovidStatusSite, layout = "l-body-outset", include = FALSE}
# DONE by Francesco
...
gtbl_sites_covid_status <- tbl_sites_covid_status %>%
  ...
  grand_summary_rows(
    ...
    formatter = fmt_number,
    decimals = 0) %>% 
  tab_header(title = glue('Summary of Covid-19 infection-related status of patients consulted/admitted to an MSF supported health service as of {format(date_max_report, "%d %b %Y")}'), 
...
gtsave(gtbl_sites_covid_status,
       file.path(path.local.msf.tables, paste0('SUMMARY-TABLE_MSF-sites_by_patients_Covid-status_', week_report, '.html'))) %>%
  invisible()
...
## Histogram of number of consultation and admissions aggregated by week and continent
...
## Histogram of weekly number of confirmed, probable, suspected, not a case and unknown overall
...
## Histogram of weekly number of confirmed, probable, suspected, not a case and unknown by continent
