...
p_pyramid_age_sex <- ggplot(tbl_pyramid, aes(x = age_10gp, y = n, fill = sex)) +
  geom_col() +
...
tbl_pyramid_continent <- dta %>% 
  drop_na(sex, age_in_years) %>% 
  filter(covid_status %in% c("Confirmed")) %>% 
  count(continent, sex, age_10gp) %>% 
  mutate(n = if_else(sex == "M", -n, n))

p_pyramid_age_sex_continent <- ggplot(tbl_pyramid_continent, aes(x = age_10gp, y = n, fill = sex)) +
...
ggsave(file.path(path.local.msf.graphs, paste0('msf', 'p_pyramid_age_sex_continent', '.png')),
...
make_cfr_tbl <- function(dta, x_var, label_var){
...
tbl_cfr_age <- make_cfr_tbl(dta, 'age_5gp', 'Age')

tbl_cfr_sex <- make_cfr_tbl(dta %>% 
                              mutate(sex = factor(sex, levels = c('F', 'M'), labels = c('Females', 'Males'))), 
                            'sex', 
                            'Sex')

tbl_cfr_comorb <- make_cfr_tbl(dta %>% 
...
tbl_risk_comcond <- make_cfr_tbl(dta_comcond, 'type_comorbidity' ,'Comorbidities')
...
tbl_non_cases_diagt <- dta %>% 
  ...
  count(main_diagnosis, outcome_status) %>% 
  pivot_wider(names_from = outcome_status, names_sort = TRUE, values_from = n, values_fill = 0) %>% 
...
gtbl_non_cases_diagt %>% 
  tab_header(
    title = 'Frequency of diagnostics among non confirmed patients who attended a Covid facility'
  )
