...
# Overview
This section will present **inforsmation of completeness of data** (number of MSF projects that are active on Covid19 care, reporting individual data (linelist), reporting aggregated data and not reporting data). Number of projects will be listed by continent and region. 
...
nb_msf_sites_aggregated <- length(unique(dta_weekly_expanded$site_name))
nb_msf_sites            <- nb_msf_sites_linelist + nb_msf_sites_aggregated

call_msf_countries <- unique(c(dta$country, dta_weekly_aggregated$country))

nb_msf_obs_linelist   <- dta %>% count() %>% pull(n)
nb_msf_obs_aggregated <- dta_weekly_expanded %>% count() %>% pull(n)
...
median_age_confirmed <- dta %>% 
  ...
  pull(age_in_years) %>% median(na.rm = TRUE)
...
Other results are presented by Covid status (confirmed, probable, suspected, not a case and unknown) and listed by country or site grouped by continent. 
...
n_max <- max(df_admit_prop$n, na.rm = TRUE)
...
missing_admission <- sum(dta$admit == "Unknown")
...
n_max <- max(df_admit_prop_continent$n, na.rm = TRUE)
...
p_epicurve_status <- dta_with_aggregated %>%
...
p_epicurve_status_continent <- dta_with_aggregated %>%
...
```{r TblAllGeneralCharacteristics, layout = "l-body-outset"}
# DONE by Francesco
...
```{r PyramidAgeContinent, fig.cap = 'Age pyramid of patients consulted/admitted to an MSF supported Covid-19 health service by continent', fig.width = 8, fig.height = 8}
# Done by Paul
...
p_pyramid_age_sex_continent <- ggplot(tbl_pyramid_continent, aes(x = age_9gp, y = n, fill = sex)) +
...
ggsave(file.path(path.local.msf.graphs, paste0('msf', 'p_pyramid_age_sex_continent_', week_report, '.png')),
       plot = p_pyramid_age_sex_continent,
       width = 8,
       height = 8)


p_pyramid_age_sex_continent
...
<!-- Age/sex pyramid -->
```{r PyramidAgeConfirmed, fig.cap = 'Age pyramid of Covid-confirmed patients consulted/admitted to an MSF supported health service', fig.width = 8, fig.height = 8}
# Done by Paul
...
p_pyramid_age_sex_confirmed <- ggplot(tbl_pyramid_confirmed, aes(x = age_9gp, y = n, fill = sex)) +
  geom_col() +
  geom_hline(yintercept = 0, colour = "black") +
  geom_text(aes(label = abs(n), hjust = if_else(n >= 0, 1.1, -0.1)), colour = "white", size = 4) +
  coord_flip() +
...
ggsave(file.path(path.local.msf.graphs, paste0('msf', 'p_pyramid_age_sex_confirmed', '_', week_report, '.png')),
       plot = p_pyramid_age_sex_confirmed,
       width = 8,
       height = 6)


p_pyramid_age_sex_confirmed
```
...
<!-- Age/sex pyramid by continent -->
```{r PyramidAgeConfirmedContinent, fig.cap = 'Age pyramid of patients consulted/admitted to an MSF supported Covid-19 health service by continent', fig.width = 8, fig.height = 8}
# Done by Paul
...
p_pyramid_age_sex_confirmed_continent <- ggplot(tbl_pyramid_confirmed_continent, aes(x = age_9gp, y = n, fill = sex)) +
  facet_wrap(~continent, ncol = 2, scales = "free_x") +
  geom_col() +
  geom_hline(yintercept = 0, colour = "black") +
  geom_text(aes(label = abs(n), hjust = if_else(n >= 0, 1.1, -0.1)), colour = "white", size = 4) +
...
ggsave(file.path(path.local.msf.graphs, paste0('msf', 'p_pyramid_age_sex_confirmed_continent', '_', week_report, '.png')),
       plot = p_pyramid_age_sex_confirmed_continent,
       width = 8,
       height = 8)


p_pyramid_age_sex_confirmed_continent
...
tbl_risk_comcond <- make_tbl_cfr(dta_comcond, 'type_comorbidity' ,'Comorbidities')
...
<!-- CFR by age-group -->
```{r CFRbyAgegroup, fig.cap = 'Case fatality risk by age-group among confirmed patients'}
...
p_age_cfr <- ggplot(df_severity, aes(x = age_group)) +
...
ggsave(file.path(path.local.msf.graphs, glue("msf_age_cfr_{week_report}.png")),
       plot = p_age_cfr,
       scale = 1.1,
       dpi = 320)
...
<!-- CFR by week overall -->
```{r LineCFRWeeks, fig.cap = 'Evolution overtime of the case fatality risk among Covid confirmed patients consulted/admitted to an MSF supported health facility'}
# DONE by Paul
...
```{r BoxPlotCFRContinent, fig.cap = 'Evolution overtime of the case fatality risk among Covid confirmed patients consulted/admitted to an MSF supported health facility, separated by continent', fig.width = 8, fig.height = 8}
# DONE by Paul
# Only including outcomes of cured or died as CFR denominator - is this ok? FG: Yes
...
p_onset_admit_delay <- ggplot(dta_delay, aes(epi_week_consultation, delay_before_admission, group = epi_week_consultation)) +
...
```{r BoxPlotLengthStay, fig.cap = 'Evolution overtime of of length of stay among Covid confirmed, probable and suspected patients in an MSF supported health facility'}
# DONE by Paul
...
boxplot_length_stay <- ggplot(tbl_length_stay, aes(epi_week_consultation, length_stay, group = epi_week_consultation)) + 
  geom_boxplot() + 
...
```{r NonCasesDiagnosis, layout="l-body-outset"}
