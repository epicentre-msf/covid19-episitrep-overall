...
**Histogram of number of consultation and admissions aggregated by week**
...
**Histogram of weekly number of consultations by continent**
...
**Histogram of weekly number of admissions by continent**
...
**Histogram of weekly number of confirmed, probable, suspected, not a case and unknown overall and by region (or continent)**
...
ggsave(file.path(path.local.msf.graphs, paste0('msf_','p_epicurve_status_continent', '_', week_report, '.png')), 
...
**Table - Frequency and proportion of patients with at least 1 comorbidity**
...
- , and by consultation/admission


# Description of confirmed patients
...
<!-- Probably by region or country of not too long -->
...
tbl_cfr_continent <- dta_cfr %>% 
  group_by(covid_status) %>% 
  summarise(total_Total = n(),
            cured_n_Total = sum(outcome_status == 'Cured', na.rm = TRUE), 
            died_n_Total  = sum(outcome_status == 'Died', na.rm = TRUE), 
            died_p_Total  = mean(outcome_status == 'Died', na.rm = TRUE)) %>% 
  ungroup()



tbl_cfr_continent_covid_status <- dta_cfr %>% 
  group_by(continent, covid_status) %>% 
  summarise(total = n(),
  ...
  pivot_wider(names_from = continent, values_from = c(total, cured_n, died_n, died_p)) %>% 
  ungroup()

gtbl_cfr_continent <- tbl_cfr_continent_covid_status %>% 
  full_join(tbl_cfr_continent) %>% 
  gt() %>% 
  cols_label(
    covid_status   = 'Covid status', 
    total_Africa   = 'Total', 
    total_Americas = 'Total',
    total_Asia     = 'Total', 
    total_Total    = 'Total',
    cured_n_Africa   = 'Cured', 
    cured_n_Americas = 'Cured',
    cured_n_Asia     = 'Cured', 
    cured_n_Total    = 'Cured', 
    died_n_Africa   = 'Died',
    died_n_Americas = 'Died', 
    died_n_Asia     = 'Died', 
    died_n_Total    = 'Died', 
    died_p_Africa   = 'CFR', 
    died_p_Americas = 'CFR',
...
gtsave(gtbl_cfr_continent, 
       file.path(path.local.msf.tables, paste0('gtbl_cfr_continent', '_', week_report, '.html')), 
       inline_css = TRUE) %>% 
  invisible()

gtsave(gtbl_cfr_continent, 
       file.path(path.local.msf.tables, paste0('gtbl_cfr_continent', '_', week_report, '.png'))) %>% 
  invisible()
...
**Table Outcome 2 - Count outcome count of cured, deaths, transferred… by Admitted/Not Admitted**
