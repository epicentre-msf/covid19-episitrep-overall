...
This section provides an overview of all patients that attended an MSF supported COvid-19 health service ...

### Summary tables of patients consulted/admitted to an MSF supported COvid-19 health service 

<!-- 
 === SUMMARY TABLE BY COUNTRY === 
Counts of patients consulted/admitted to an MSF supported health service by Covid status (confirmed, probable, suspected, not a case and unknown) listed by country
-->
...
Counts of patients consulted/admitted to an MSF supported health service by Covid status (confirmed, probable, suspected, not a case and unknown) 
-->
...
### Histograms of number of consultation and admissions aggregated by week
...
### Histograms of weekly number of Consultations/Admission by Covid status
...
gtbl_cfr_status_continent %>% 
  tab_header(
    ...
    subtitle = 'In the denominator only patients with outcome as cured or deaths'
  )
...
### Characteristics of Covid confirmed patients
...
### Case fatality risk among confirmed patients by continent
...
Consider to To be stratified by continent 
-->
```{r}
# DONE by Francesco
