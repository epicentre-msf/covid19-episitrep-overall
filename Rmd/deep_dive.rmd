...
sf_trend_14d <- df_countries %>% 
  ...
  mutate(trend_cases = factor(trend_cases, 
                              levels = c("Increasing", "Likely increasing",
                                         "Stable", "Likely decreasing",
                                         "Decreasing")))
...
map_case_trend_14d <- ggplot(sf_trend_14d) + 
  geom_sf(aes(fill = trend_cases), size = .1, alpha = 0.8) + 
  scale_fill_manual(
    name = NULL, 
    values = values, 
    drop = FALSE, 
...
```{r fig-epicurves}
iso_a3_increasing <- tbl_coeff_30d %>% 
...
epicurves <- ggplot(dta_jhu_increasing, 
                    aes(x = date, y = cases)) + 
...
preds <- ggplot(tbl_preds_cases_30d, 
                aes(x = date, y = cnt)) + 
  facet_wrap(~ country, scales = "free_y", ncol = 4) + 
