...
# -- Linear model trends - last 12 days 

lst_mdl_cases_12d <- list()
...
for(i in names(lst_dta_jhu)){
  mdl <- linear_trend_deepdive(dta = lst_dta_jhu[[i]], 
                               ...
                               n_days = 12, 
                               last_date = date_max_report)
  lst_mdl_cases_12d[[i]] <- mdl
}


lst_mdl_deaths_12d <- list()
...
lst_preds_cases_12d <- lapply(lst_mdl_cases_12d, function (x) x[['preds']])

lst_preds_deaths_30d <- lapply(lst_mdl_deaths_30d, function (x) x[['preds']])
lst_preds_deaths_12d <- lapply(lst_mdl_deaths_12d, function (x) x[['preds']])
...
tbl_coeff_cases_12d <- lapply(lst_mdl_cases_12d, function (x) x[['coeffs']]) %>% 
  tibble::enframe(name = "iso_a3") %>% 
  unnest(cols = c(value)) %>% 
  mutate(
    trend = case_when(
...
tbl_coeff_deaths_12d <- lapply(lst_mdl_deaths_12d, function (x) x[['coeffs']]) %>% 
  tibble::enframe(name = "iso_a3") %>% 
  unnest(cols = c(value))
...
sf_case_trend_12d <- df_countries %>% 
  select(iso_a3, country) %>% 
  left_join(tbl_coeff_cases_12d) %>% 
  full_join(
    select(sf_mercator, iso_a3),
    by = "iso_a3") %>% 
  st_as_sf()
...
map_case_trend_12d <- ggplot(sf_case_trend_12d) + 
  geom_sf(aes(fill = trend), size = .1, alpha = 0.8) + 
  scale_fill_manual(
    name = NULL, 
    values = RdAmGn, 
...
## Combine maps
map_cases_trends <- map_case_trend_30d + map_case_trend_12d + 
  plot_layout(ncol = 2, guides = 'collect') & theme(legend.position = 'bottom')
