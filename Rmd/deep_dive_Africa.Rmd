...
path.local.deepdive.Africa <- file.path(path.local.worldwide, 'deepdive_Africa')
...
dta_ecdc <- get_owid_jhcsse() %>% 
  tidyr::drop_na(iso_a3) %>% 
  filter(between(date, left = NULL, right = date_max) & (continent == 'Africa'))
...
Data source: [JHU CSSE data](https://github.com/owid/covid-19-data/tree/master/public/data) updated until `r format(date_max, '%d %B %Y')`
...
```{r, fig.cap = paste("TREEMAP: Total number and incidence proportion of associated deaths per country in", combine_words(grp_region_NW))}
...
```{r, fig.cap = paste("TREEMAP: Total number and incidence proportion of associated deaths per country in", combine_words(grp_region_EMS))}
...
```{r, fig.cap = paste("Number of confirmed associated deaths since the beginning of the epidemic and in the last 14 days per country in", combine_words(grp_region_NW))}
...
```{r, fig.cap = paste("Number of confirmed associated deaths since the beginning of the epidemic and in the last 14 days per country in", combine_words(grp_region_EMS))}
...
for(i in names(lst_dta_ecdc)){
  mdl <- linear_trend(dta = lst_dta_ecdc[[i]], series = 'cases', n_days = 12, last_date = date_max)
...
for(i in names(lst_dta_ecdc)){
  mdl <- linear_trend(dta = lst_dta_ecdc[[i]], series = 'deaths', n_days = 12, last_date = date_max)
...
dta_ecdc_increasing <- dta_ecdc %>% 
  filter(iso_a3 %in% iso_a3_increasing)


epicurves <- ggplot(dta_ecdc_increasing, aes(x = date, y = cases)) + 
