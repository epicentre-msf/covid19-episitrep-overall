...
# Date limits using date of consultation
dta_linelist <- dta_linelist %>% 
  filter(between(date_consultation, left = NULL, right = date_max_report) | is.na(date_consultation))
...
gtbl_consultation_continent <- tbl_consultation_continent %>% 
  ...
  fmt_number(
    ...
    decimals = 1) %>% 
  tab_header(title = "Admitted by continent")
...
tbl_age_sex_continent <- dta_linelist %>% 
...
recode_unknown_to_na <- function(x){
  na_if(x,'Unknown')
}
...
dta_comcond <- dta_comcond %>% 
  ...
  mutate(across(c(starts_with('Comcond_'),'hiv_status','hypertension', 'malaria', 'malnutrition', 'smoking', 'tb_active'), recode_unknown_to_na))
...
tbl_comcond_status <- names(dta_comcond)[4:ncol(dta_comcond)] %>% 
  map_df(~{
    dta_comcond %>% 
      drop_na(any_of(.x)) %>% 
      make_tbl_prop(.x, 'covid_status', drop_levels = TRUE) %>% 
      rename('value'= .x) %>% 
      mutate(
        name_var = .x)
    }) %>% 
...
gtbl_comcond_status <- tbl_comcond_status %>% 
  gt() %>% 
  cols_hide('value') %>% 
  cols_move_to_start(columns = vars(name_var)) %>% 
  cols_label(
    name_var = "Comorbidities/Underlying conditions", 
    n_Confirmed    = 'n', 
    n_Probable     = 'n',
    n_Suspected    = 'n', 
    `n_Not a case` = 'n', 
...
tbl_comcond_N <- names(dta_comcond)[4:ncol(dta_comcond)] %>% 
  map_df(~{
    dta_comcond %>% 
      drop_na(any_of(.x)) %>% 
      group_by(covid_status) %>% 
  ...
  pivot_wider(names_from = covid_status, values_from = N, names_prefix = 'N_') %>% 
  adorn_totals(where = 'col', name = 'N_Total')

left_join(tbl_comcond_status, tbl_comcond_N) %>% 
  gt() %>% 
  cols_hide(
    columns = vars(value)) %>% 
  cols_merge(
    columns = vars(n_Confirmed, N_Confirmed), 
    pattern = "{1}/{2}") %>% 
  cols_merge(
    columns = vars(n_Probable, N_Probable), 
    pattern = "{1}/{2}") %>% 
  cols_merge(
    columns = vars(n_Suspected, N_Suspected), 
    pattern = "{1}/{2}") %>% 
  cols_merge(
    columns = vars(`n_Not a case`, `N_Not a case`), 
    pattern = "{1}/{2}") %>% 
  cols_merge(
    columns = vars(`n_Not a case`, `N_Not a case`), 
    pattern = "{1}/{2}") %>% 
...
### Case fatality among confirmed patients
...
tbl_cfr_type_comcond <- dta_comcond %>% 
  drop_na() %>% 
  make_tbl_cfr(x_var = 'type_comorbidity') %>% 
