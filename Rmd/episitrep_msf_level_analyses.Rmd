...
tbl_prop_symp <- tibble(covid_status = tbl_sum[[1]])

for(i in names(names_sympt)) {
  tbl_prop_symp[i] <- cbind(tbl_sum[[i]] / tbl_count[[i]])
}

tbl_prop_symp <- tbl_prop_symp %>%
  pivot_longer(-covid_status, names_to = 'symtpoms') %>%
  pivot_wider(names_from = covid_status, values_from = value)

tbl_sympt_all <- tbl_freq %>%
  left_join(tbl_prop_symp, by = 'symtpoms', suffix = c(".freq", ".prct")) %>%
  arrange(symtpoms)
...
tbl_severity_status <- names(dta_severity)[2] %>% 
  map_df(~{
    dta_severity %>% 
      drop_na(any_of(.x)) %>% 
      tbl_prop(.x, 'covid_status', drop_levels = TRUE)
...
# DONE by Francesco
# function tbl_cfr in utils_vis.R
...
tbl_cfr_type_comcond <- dta_comcond %>% 
  drop_na() %>% 
  tbl_cfr(x_var = 'type_comorbidity') %>% 
...
tbl_non_cases_diagt <- tbl_prop(dta_non_cases_diagt, 'main_diagnosis', 'outcome_status')
