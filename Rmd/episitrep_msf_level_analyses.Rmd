...
## Case fatality by age, sex and number of comorbidities among confirmed patients
...
dta_cfr_sex <- dta_linelist %>%
   filter(nd_MSF_covid_status == "Confirmed") %>% 
...
dta_cfr_comcond <- dta_linelist %>% 
  filter(ind_Comcond_count < 9) %>% # This is a temporal rapid solution to remove cases with ALL comorbidities that might be a recording error
  select(continent, ind_MSF_covid_status, ind_outcome_patcourse_status, ind_Comcond_count, ind_Comcond_01, starts_with('ind_Comcond_'), ind_MSF_hiv_status, ind_MSF_hypertension, ind_MSF_tb_active, ind_MSF_malaria, ind_MSF_malnutrition, ind_MSF_smoking) %>% 
  mutate(
