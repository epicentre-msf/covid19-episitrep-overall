...
dta_linelist <- get_msf_linelist(force = TRUE) %>% 
  covidutils::prepare_msf_dta(shorten_var_names = TRUE) %>% 
...
dta_age_sex <- dta_linelist %>% 
  mutate(sex = factor(sex, levels = c('F', 'M'), labels = c('Female', 'Male')) %>% fct_explicit_na(na_level = "(Unknown)")) 
...
tbl_sex_ratio <- dta_linelist %>% 
  ...
  transform(sex = factor(sex, levels = c('M', 'F'), labels = c('Male', 'Female')) %>% fct_explicit_na(na_level = "(Unknown)")) %>% 
  group_by(continent) %>% 
  count(sex) %>% 
...
gtbl_age_sex_outcome <- tbl_age_sex_outcome %>% 
  ...
  cols_align(
    align = 'left', 
    columns = vars(outcome_status, age__prcl)) %>% 
  cols_align(
    align = 'right', 
  ...
  tab_style(
    style = list(
      cell_text(align = 'left')),
    locations = cells_column_labels(columns = vars(outcome_status, age__prcl))) %>%
  tab_options(
    column_labels.font.weight = 'bold', 
    data_row.padding = px(2))
