...
author: Francesco Grandesso (Epicentre) <mailto:francesco.GRANDESSO@epicentre.msf.org>
...
nb_msf_confirmed_with_comorbidity <- with(dta_linelist, sum(ind_MSF_covid_status == 'Confirmed' & Comcond_count > 0, na.rm = TRUE))
...
tbl_general_by_status <- dta_linelist %>% 
  select(ind_MSF_covid_status, age_in_years, age_5gp, patinfo_sex, Comcond_01) %>% 
  ...
  summarise(
    ...
    com_presence  = freq_prct(Comcond_01, 1))
...
dta_cfr_comcond <- dta_linelist %>% 
  select(continent, ind_MSF_covid_status, ind_outcome_patcourse_status, Comcond_count, Comcond_01, starts_with('ind_Comcond_'), ind_MSF_hiv_status, ind_MSF_hypertension, ind_MSF_tb_active, ind_MSF_malaria, ind_MSF_malnutrition, ind_MSF_smoking) %>% 
  mutate(
    ind_Comcond_other = case_when(
      !is.na(ind_Comcond_other) ~ 'Yes', 
      TRUE ~ NA_character_)) %>% 
  filter(ind_MSF_covid_status == 'Confirmed', ind_outcome_patcourse_status %in% c('Cured', 'Died')) %>% 
  pivot_longer(-c(ind_MSF_covid_status, ind_outcome_patcourse_status, continent, Comcond_count, Comcond_01), names_to = "type_comorbidity") %>%
  mutate(
    type_comorbidity = factor(type_comorbidity, levels = df_labels_comcond$levels, labels = df_labels_comcond$labels)) %>% 
