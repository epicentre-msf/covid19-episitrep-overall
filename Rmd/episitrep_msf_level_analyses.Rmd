...
gtbl_age_sex_continent <- tbl_age_sex_continent %>% 
  ...
  fmt_number(
    columns = 2:4, 
    decimals = 0) %>% 
  tab_header(title = 'Median age (years) of patients by continent, all patients consulted/admitted')
...
gtbl_sex_ratio <- tbl_sex_ratio %>% 
  ...
  cols_label(
    continent = '',
    ratio_mf = 'M/F ratio') %>% 
  tab_header(title = 'Sex ratio of all patients consulted/admitted by continent')
...
tbl_comcond_status <- dta_linelist %>% 
  select(covid_status, Comcond_present_01) %>% 
  mutate(covid_status = na_if(covid_status, "Unknown") %>%  fct_explicit_na()) %>% 
  mutate(Comcond_present_01 = factor(Comcond_present_01, levels = c(1, 0), labels = c('Yes', 'No')) %>%  fct_explicit_na()) %>% 
  group_by(covid_status) %>% 
  count(Comcond_present_01) %>% 
  pivot_wider(names_from = Comcond_present_01, values_from = n) %>% 
  adorn_totals(c('row', 'col')) %>% 
  mutate(
    p_Yes = round(Yes / (Total - `(Missing)`) * 100, 1)
  )
