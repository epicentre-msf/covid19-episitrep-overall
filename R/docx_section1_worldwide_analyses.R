...
# 3
my_doc <- add_par_normal(
  sprintf('Most of countries worldwide have been reporting new cases with an increasing or stable trend. %s countries reported an increasing trend this week (compared to XX last week) (Figure 1 - Trends in cases).', 
...
# 5 - Incidence
my_doc <- add_par_normal(
  sprintf('Since the beginning of the epidemic, countries presenting the highest cumulative incidences are in North and South America, Europe, Middle East, and some countries in central America). In Africa roughly half of the countries remain below the threshold of 10 reported cases per 100,000 population, while the other half is below the threshold of 100 reported cases per 100,000 population, with the exception of Mauritania, Gabon and South Africa (Figure 2).'))
...
# 1
my_doc <- add_par_normal(
  sprintf('Similarly to the number of cases, countries that reported the highest numbers of Covid-19 associated deaths are the USA, Brazil and some countries in Europe. NAME OF THE COUNTRIES also reached the threshold of 10,000 Covid19 associated deaths. About half of African countries are now reporting more than 100 deaths (Figure 3 - Deaths count).'))
