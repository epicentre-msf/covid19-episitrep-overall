...
path <- max(fs::dir_ls("~/MSF/GRP-EPI-COVID-19 - NCoVEpi/data/linelist/world", regexp = "[.]rds$"))

dat <- readr::read_rds(path) 
```
...
- `age_10gp`: groups the `age_in_years` variable into the following bins: 0-9, 10-19, 20-29, 30-39, 40-49, 50-59, 60-69, 70-79, 80-89, 90+
...
values: `r paste(sort(unique(dat$patinfo_sex)), collapse = ", ")`

#### Covid Status

linelist variable: `MSF_covid_status`  
values: `r paste(sort(unique(dat$MSF_covid_status)), collapse = ", ")`

#### Visit Type

linelist variable: `MSF_visit_type`  
values: `r paste(sort(unique(dat$MSF_visit_type)), collapse = ", ")`

#### Final Outcome

linelist variable: `outcome_patcourse_status`  
values: `r paste(sort(unique(dat$outcome_patcourse_status)), collapse = ", ")`
...
Prior to computing indicators, it is common to filter the data to either confirmed, or confirmed, probable or suspect patients only. Please refer to the analysis plan for details on what filters are used in each table or graphic.
